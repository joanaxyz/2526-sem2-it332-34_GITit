...
### Launcher Hero & Blade Nav (home hub)
The hub's top chrome is a single wide glass blade — `\ Home | Tower /` (`.hub-nav`, ~50rem, angled ends via `--nav-cut`, blur 14) floating over the environment art with the logo chip and wallet/profile loose in the top corners; there is no header bar. The hero composes bottom-heavy like a game launcher: a diagonal-hatched glass band (`.hub-hero-band`) shelves the hero's bottom edge; the glass wordmark plate (`.hub-logo-frame`, blur 16, translucent fill so the art reads through) straddles that edge at center, flanked by the streak/achievement plaques on the same line (`.hub-hero-ledge`); the chamfered CTA buttons sit below the hero on the page, also glass. Glass on this surface is material, not decoration: blade, plate, plaques, CTAs — all over live art.
