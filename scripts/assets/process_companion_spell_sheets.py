...
# Image generators sometimes draw a bright separator grid over the requested
# 5x5 sheet. Chroma keying correctly removes the backing card but keeps those
# white rules as opaque effect pixels, which later become a rectangle inside
# every normalized frame. Only treat a row/column as a separator when a
# near-neutral bright line covers almost the entire sheet.
GRID_LINE_MIN_COVERAGE = 0.8
GRID_LINE_SEARCH_RADIUS = 4
GRID_LINE_CLEAR_RADIUS = 3
...
COMMANDS = [
    ...
    {"name": "tag", "tint": "indigo", "motif": "seal", "playback": "target", "anchor": "feet"},
    {
        "name": "rev-list",
        "tint": "violet",
        "motif": "ancestry",
...
CONCEPT_REFERENCE: str | None = (
    "/cosmetics/companion/blue/effects/skill-flames/_blue-flame-reference-concept.png"
)
...
def _frame_grid_bounds(
    width: int, height: int
) -> tuple[list[tuple[int, int]], list[tuple[int, int]]]:
    return fixed_axis_bounds(width, GRID_COLUMNS), fixed_axis_bounds(height, GRID_ROWS)


def clear_detected_bright_grid_lines(im: Image.Image) -> Image.Image:
    """Remove generator-authored white grid rules without touching VFX highlights.

    The detector is deliberately narrow: it only searches near expected 5x5
    separators and requires a bright, nearly neutral row/column to span at least
    80% of the raw sheet. A local white spark or ice blade cannot trigger it.
    """
    import numpy as np
    ...
    bright_neutral = (rgb.min(axis=2) >= 205) & ((rgb.max(axis=2) - rgb.min(axis=2)) <= 42)
    height, width = bright_neutral.shape

    def separator_coordinates(length: int, count: int) -> list[int]:
        return [round(index * length / count) for index in range(count + 1)]

    rows_to_clear: set[int] = set()
    for expected in separator_coordinates(height, GRID_ROWS):
        center = min(height - 1, max(0, expected))
        start = max(0, center - GRID_LINE_SEARCH_RADIUS)
        end = min(height, center + GRID_LINE_SEARCH_RADIUS + 1)
        for row in range(start, end):
            if float(bright_neutral[row, :].mean()) < GRID_LINE_MIN_COVERAGE:
                continue
            rows_to_clear.update(
                range(
                    max(0, row - GRID_LINE_CLEAR_RADIUS),
                    min(height, row + GRID_LINE_CLEAR_RADIUS + 1),
                )
            )

    columns_to_clear: set[int] = set()
    for expected in separator_coordinates(width, GRID_COLUMNS):
        center = min(width - 1, max(0, expected))
        start = max(0, center - GRID_LINE_SEARCH_RADIUS)
        end = min(width, center + GRID_LINE_SEARCH_RADIUS + 1)
        for column in range(start, end):
            if float(bright_neutral[:, column].mean()) < GRID_LINE_MIN_COVERAGE:
                continue
            columns_to_clear.update(
                range(
                    max(0, column - GRID_LINE_CLEAR_RADIUS),
                    min(width, column + GRID_LINE_CLEAR_RADIUS + 1),
                )
            )

    if not rows_to_clear and not columns_to_clear:
        return out
    if rows_to_clear:
        arr[sorted(rows_to_clear), :, 3] = 0
    if columns_to_clear:
        arr[:, sorted(columns_to_clear), 3] = 0
...
def remove_chroma_key_background(im: Image.Image) -> Image.Image:
    ...
    high = 92
    alpha = diff.point(
        lambda v: 0 if v <= low else 255 if v >= high else round((v - low) * 255 / (high - low))
    )
...
def shifted_frame(
    frame: Image.Image, dx: int, dy: int, context: str = "frame shift"
) -> Image.Image:
    frame = frame.convert("RGBA")
...
def owned_frame_sources(raw_path: Path) -> list[OwnedFrameSource]:
    ...
    y_edges = np.array(
        [cell_boxes[row * GRID_COLUMNS][1] for row in range(GRID_ROWS)] + [cell_boxes[-1][3]]
    )
    ...
    for index, cell_box in enumerate(cell_boxes):
        ...
        copy_mask = (
            ndimage.binary_dilation(frame_mask, structure=structure, iterations=1) & alpha_present
        )
...
def alpha_composite_clipped(
    ...
    base.alpha_composite(
        overlay.crop((src_left, src_top, src_right, src_bottom)), (x + src_left, y + src_top)
    )
...
def render_frame_sources(
    ...
    return [
        render_frame_source(source, scale, f"{context} frame {index}")
...
def detect_projectile_nose_anchor(frame: Image.Image) -> FrameAnchor | None:
    ...
    for y in range(y0, y1):
        for x in range(x0, x1):
            ...
            weight = (
                alpha_weight
                if ANCHOR_MODE == "alpha"
                else alpha_weight * (0.25 + bright_weight) ** 2.25
            )
    ...
    nose_y = (
        weighted_median(nose_band)
        if nose_band
        else weighted_median([(y, weight) for _, y, weight in weighted_pixels])
    )
    ...
    for y in range(y0, y1):
        for x in range(x0, x1):
            ...
            weight = (
                alpha_weight
                if ANCHOR_MODE == "alpha"
                else alpha_weight * (0.40 + bright / 255.0) ** 1.4
            )
            xs.append((x, weight))
            ys.append((y, weight))
            total_weight += weight
...
def registration_phase_ranges(
    ...
    return [(start, end) for start, end in zip(cuts, cuts[1:], strict=False) if start < end]
...
def stabilize_registration_shifts(
    ...
    flight_bounds = (
        projectile_phase_bounds(launch_start, impact_start) if playback == "projectile" else None
    )
...
def _feet_registration_plan(
    ...
    def pools(
        anchors: list[FrameAnchor | None],
    ) -> tuple[dict[int, FrameAnchor], dict[int, FrameAnchor]]:
        ...
        reliable = {
            i: a for i, a in enumerate(anchors) if a is not None and is_reliable_anchor(a, mw, mp)
        }
    ...
    const_dx = clamp_int(
        round(FRAME * 0.5 - seq_center_x), -MAX_REGISTRATION_SHIFT, MAX_REGISTRATION_SHIFT
    )
    ...
    for index, anchor in enumerate(anchors):
        ...
        dy = clamp_int(
            round(GROUND_ANCHOR_Y - anchor.y), -MAX_REGISTRATION_SHIFT, MAX_REGISTRATION_SHIFT
        )
...
def _projectile_target_plan(
    ...
    for index, detected in enumerate(anchors):
        ...
        dx = clamp_int(
            round(target_x - detected.x), -MAX_REGISTRATION_SHIFT, MAX_REGISTRATION_SHIFT
        )
        dy = clamp_int(
            round(target_y - detected.y), -MAX_REGISTRATION_SHIFT, MAX_REGISTRATION_SHIFT
        )
        shift_by_index[index] = (dx, dy)
...
def keyed_frames(raw_path: Path, fit_margin: int = FIT_MARGIN_PX) -> list[Image.Image]:
    ...
    return frames_from_owned_sources(
        raw_path, sources, compute_prefit_scale(sources, margin=fit_margin)
    )
...
def combine_layer_frames(back: list[Image.Image], front: list[Image.Image]) -> list[Image.Image]:
    ...
    for a, b in zip(back, front, strict=False):
        merged = a.copy()
...
def manifest_entry(name: str, spec: dict[str, object], layered: bool = False) -> dict[str, object]:
    ...
    entry: dict[str, object] = {
        ...
        "src": f"{EFFECT_BASE_URL}/{name}_front.png"
        if layered
        else f"{EFFECT_BASE_URL}/{name}.png",
        "fps": FPS,
        "loops": False,
        "frameWidth": FRAME,
        "frameHeight": FRAME,
...
def remeasure_place_anchors() -> None:
    ...
    print(
        f"Remeasured placeAnchor for {measured}/{len(sprites)} sprites in {OUT_DIR / 'manifest.json'}"
    )


def load_manifest_and_diagnostics(
    known_names: set[str],
