...
When a cleaned root sprite has accidentally filled holes that are already
transparent in its raw source, use --restore-alpha-holes. This copies only
enclosed transparent regions from raw to root and preserves every current
destination pixel outside those holes.

When the current root sprite itself still contains enclosed near-white islands,
use --clean-current-white-holes with --min-hole-size. This removes only those
islands from the current destination instead of rebuilding it from raw.
...
class CleanResult:
    ...
    raw_path: Path | None
    wrote: bool
    restored_alpha_holes: bool
    cleaned_current_white_holes: bool
    cleaned_current_connected_white: bool
...
def enclosed_white_mask(
    ...
    """Return enclosed near-white islands without touching exterior whites.

    The current-root cleanup mode must preserve every already-cleaned exterior
    edge. Run the connected-white detector frame by frame and subtract its
    exterior-only result, leaving only enclosed components at or above
    ``min_hole_size``.
    """
    width, height = rgba.size
    if (
        (grid_columns > 1 or grid_rows > 1)
        ...
        for row in range(grid_rows):
            for column in range(grid_columns):
                ...
                frame_mask = _enclosed_white_mask_for_image(
    ...
    return _enclosed_white_mask_for_image(
        rgba,
        tolerance=tolerance,
        alpha_threshold=alpha_threshold,
        min_hole_size=min_hole_size,
...
def _enclosed_white_mask_for_image(
    rgba: Image.Image,
    tolerance: int,
    alpha_threshold: int,
    min_hole_size: int,
    ...
    def seed(index: int) -> None:
        if traversable[index] and not exterior[index]:
            exterior[index] = 1
            queue.append(index)
    ...
    while queue:
        ...
        if x > 0:
            neighbor = index - 1
            if traversable[neighbor] and not exterior[neighbor]:
                exterior[neighbor] = 1
                queue.append(neighbor)
        if x < width - 1:
            neighbor = index + 1
...
def enclosed_transparent_mask(
    ...
    """Return transparent source regions enclosed within each sprite frame."""
    width, height = rgba.size
    if (
        (grid_columns > 1 or grid_rows > 1)
        and width % grid_columns == 0
        ...
        for row in range(grid_rows):
            for column in range(grid_columns):
                left = column * frame_width
                top = row * frame_height
                frame_mask = _enclosed_transparent_mask_for_image(
                    rgba.crop((left, top, left + frame_width, top + frame_height)),
                    alpha_threshold=alpha_threshold,
                    min_hole_size=min_hole_size,
                )
    ...
    return _enclosed_transparent_mask_for_image(
        rgba,
        alpha_threshold=alpha_threshold,
        min_hole_size=min_hole_size,
    )
...
def _enclosed_transparent_mask_for_image(
    rgba: Image.Image,
    alpha_threshold: int,
    min_hole_size: int,
) -> bytearray:
...
def transparent_hole_fringe_mask(
    ...
    width, height = rgba.size
    if grid_columns > 1 and grid_rows > 1 and width % grid_columns == 0 and height % grid_rows == 0:
        frame_width = width // grid_columns
        frame_height = height // grid_rows
        full_mask = bytearray(width * height)
        for row in range(grid_rows):
...
def clean_png(
    ...
    grid_columns: int,
    grid_rows: int,
    restore_alpha_holes: bool,
    clean_current_white_holes: bool,
    edge_defringe: bool,
    edge_defringe_radius: int,
    dry_run: bool,
    clean_current_connected_white: bool = False,
) -> CleanResult:
    with Image.open(task.source) as image:
        ...
        if preserve_destination:
            if not task.destination.exists():
                raise SystemExit(
                    "current-destination cleanup requires an existing destination: "
                    f"{task.destination}"
                )
            ...
            if restore_alpha_holes:
                remove_mask = enclosed_transparent_mask(
                    source_rgba,
                    alpha_threshold=alpha_threshold,
                ...
                remove_mask = bytearray(
                    1 if remove_mask[index] and a > alpha_threshold else 0
                    for index, (_, _, _, a) in enumerate(image_data(rgba))
                )
            elif clean_current_white_holes:
                remove_mask = enclosed_white_mask(
                    rgba,
                    tolerance=tolerance,
                    alpha_threshold=alpha_threshold,
                    min_hole_size=min_hole_size,
        ...
        recolored = 0
        if edge_defringe and not preserve_destination:
    ...
    if not dry_run:
        ...
        if removed or recolored:
            ...
            wrote = True
        elif not preserve_destination and task.source.resolve() != task.destination.resolve():
    ...
    return CleanResult(
        ...
        wrote=wrote,
        restored_alpha_holes=restore_alpha_holes,
        cleaned_current_white_holes=clean_current_white_holes,
        cleaned_current_connected_white=clean_current_connected_white,
    )
...
def tasks_for_raw_directory(directory: Path, include_portraits: bool = False) -> list[SpriteTask]:
    ...
    return [
        SpriteTask(
            source=path, destination=destination_dir / path.name, raw_path=path, stage_action=None
        )
        for path in direct_pngs(directory, include_portraits)
    ]
...
def print_result(result: CleanResult, dry_run: bool) -> None:
    ...
    if result.removed_pixels == 0 and result.recolored_pixels == 0:
        action = (
            "would copy "
            if dry_run
            and not result.restored_alpha_holes
            and not result.cleaned_current_white_holes
            and not result.cleaned_current_connected_white
            and result.source != result.destination
            else "unchanged"
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        default=4,
        help=(
            "minimum enclosed transparent component size before fringe cleanup or "
            "alpha-hole restoration applies (default: 4)"
        ),
    )
    parser.add_argument(
        "--hole-fringe-min-rgb",
    ...
    current_cleanup_group = parser.add_mutually_exclusive_group()
    current_cleanup_group.add_argument(
        "--restore-alpha-holes",
        action="store_true",
        help=(
            "preserve the current destination artwork and restore only enclosed "
            "transparent regions from its raw source"
        ),
    )
    current_cleanup_group.add_argument(
        "--clean-current-white-holes",
        action="store_true",
        help=(
            "preserve the current destination artwork and remove only enclosed "
            "near-white islands at or above --min-hole-size"
        ),
    )
    current_cleanup_group.add_argument(
        "--clean-current-connected-white",
        action="store_true",
        help=(
            "preserve the current destination artwork and remove only near-white "
            "pixels connected to its transparent background"
        ),
    )
    ...
    if args.clean_current_white_holes and args.min_hole_size == 0:
        raise SystemExit("--clean-current-white-holes requires --min-hole-size >= 1")
    ...
    if (
        args.restore_alpha_holes
        or args.clean_current_white_holes
        or args.clean_current_connected_white
    ):
        tasks = [task for task in tasks if task.destination.exists()]
    ...
    for task in tasks:
        result = clean_png(
            ...
            restore_alpha_holes=args.restore_alpha_holes,
            clean_current_white_holes=args.clean_current_white_holes,
            edge_defringe=args.edge_defringe,
            edge_defringe_radius=args.edge_defringe_radius,
            dry_run=args.dry_run,
            clean_current_connected_white=args.clean_current_connected_white,
        )
