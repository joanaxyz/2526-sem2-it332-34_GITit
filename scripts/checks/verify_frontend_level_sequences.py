...
def generate_cheat_sheet(path: Path) -> dict[str, int]:
    ...
    for story_slug in STORY_ORDER:
        ...
        chapters = list(
            Chapter.objects.filter(story=story, is_published=True).order_by(
            ...
            for level_index, level in enumerate(chapter_levels, start=1):
                ...
                for wave_index, wave in enumerate(waves, start=1):
                    ...
                    for variant in variants:
                        ...
                        grouped[key].append(variant.label or variant.case_id or variant.slug)
                    for sequence_index, ((commands, workspace_json), labels) in enumerate(
                        grouped.items()
                    ):
...
def wait_for_run_update(run_id: int, *, previous_wave_id: int, previous_count: int) -> AdventureRun:
    ...
    while time.monotonic() < deadline:
        ...
        if run.current_wave_id != previous_wave_id or run.command_count > previous_count:
            return run
        time.sleep(0.2)
    return AdventureRun.objects.select_related("current_wave", "selected_variant", "level").get(
        pk=run_id
    )
...
def grant_chapter_trial_gate(player: Player, chapter: Chapter) -> tuple[int, int]:
    trial_count = 0
    level_count = 0
    levels = ChallengeLevel.objects.filter(chapter=chapter, is_published=True).prefetch_related(
        "trials"
    )
...
def exercise_run(browser: Browser, run: AdventureRun) -> tuple[bool, str, AdventureRun]:
    ...
    while run.status == "started":
        ...
        deadline = time.monotonic() + LOCAL_UI_UPDATE_TIMEOUT
        while time.monotonic() < deadline:
            run = AdventureRun.objects.select_related(
                "current_wave", "selected_variant", "level"
            ).get(pk=run_id)
            if run.status != "started" or run.current_wave_id != wave_id:
                break
            time.sleep(0.25)
    ...
    *,
    ...
    for line in path.read_text(encoding="utf-8", errors="replace").splitlines():
        ...
        if record.get("username") != username or record.get("session") != session:
            continue
        key = (
            str(record.get("story")),
            int(record.get("chapter", 0)),
            str(record.get("level_slug")),
        )
        latest[key] = record
    return latest
            ...
            if existing is not None:
                ...
                record = {
                    ...
                    "detail": (
                        prior.get(
                            "detail",
                            "Earlier frontend sequence failure; test-only gate bypass retained.",
                        )
                        if is_bypass
                ...
                print(
                    f"[{checked:03d}/{total}] {state} · {story_slug} · C{chapter.number:02d} · {level.title}",
                    flush=True,
                )
                continue
