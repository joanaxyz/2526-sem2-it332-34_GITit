...
export function deriveBattleEventsFromCommandOutcome(input: DeriveFromCommandOutcomeInput): BattleBlock {
  ...
  const monsters = baseMonsters.map((m, index) => {
    if (index !== 0) return { ...m }
    // An invalid/unprocessable command cannot change repository progress, so it
    // must never reconstruct the opponent from a zeroed rule payload and heal it.
    if (!outcome.processed) return { ...m }
