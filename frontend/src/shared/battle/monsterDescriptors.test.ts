...
describe('definitionForMonster sprite sheets', () => {
  ...
  it('matches compacted monster sheet frames in the registry', () => {
    const monsters = getStoryWorld(DEFAULT_STORY_WORLD_SLUG).battle.monsters

    expect(monsters['bone-lancer'].sprites.idle.frameWidth).toBe(500)
    expect(monsters['bone-lancer'].sprites.idle.frameHeight).toBe(307)
  })
})
