...
export function bindBattleAudioVisibility(target: Document = document): () => void {
  const handleVisibilityChange = () => {
    if (target.hidden) stopRunLoop(0)
  }
...
function playLoopFor(src: string, volume: number, durationMs: number): void {
  if (!canUseAudio() || !battleSoundEffectsEnabled || isDocumentHidden()) return
  stopRunLoop(90)
  const base = loadAudio(src)
  const audio = base.cloneNode(true) as HTMLAudioElement
  audio.volume = volume
