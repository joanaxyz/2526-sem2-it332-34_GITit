import { Castle, DoorOpen, RefreshCcw, Sparkles } from 'lucide-react'
...
// The single outcome surface shown when an adventure run ends. There is only one
// of these (no separate completion modal): primary runs celebrate passing and
// the challenge unlock; replays are uncounted free-play, so they recap the run
// without touching the pass framing.
export function AdventureRunOutcome({
  run,
  onRestart,
  onBackToTower,
}: {
  ...
  const title = isReplay ? 'Replay complete' : passed ? 'Adventure passed' : 'Adventure ended'
      ...
      <CardContent className="space-y-5">
        {isReplay ? (
          <div className="flex items-center gap-2 rounded-md border border-border bg-secondary/40 px-3 py-2 text-sm text-muted-foreground">
            ...
            <span className="font-semibold text-foreground">Free play</span>
            <span>— this run doesn&apos;t change your saved progress.</span>
          </div>
        ) : passed ? (
