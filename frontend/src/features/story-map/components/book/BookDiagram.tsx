...
import type { BookBlock, BookDiagramEdge, BookDiagramNode } from '@/features/story-map/components/book/bookTypes'

const ACCENT_RGB: Record<string, string> = {
  cyan: 'var(--theme-primary-rgb)',
  purple: 'var(--theme-challenge-rgb)',
  muted: 'var(--theme-primary-soft-rgb)',
}
...
// Authored diagrams render as SVG so they keep the story world's neon look at any size.
    ...
    <svg className="book-diagram-svg" viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Flow diagram">
      <defs>
        <marker id="book-flow-arrow" markerWidth="9" markerHeight="9" refX="7" refY="4.5" orient="auto">
          <path d="M0 0 L9 4.5 L0 9 Z" fill="rgba(var(--theme-spark-rgb), 0.85)" />
        </marker>
      </defs>
      {edges.map((edge) => {
        ...
        return (
          <g key={`${edge.from}-${edge.to}`}>
            <line
              ...
              x2={x2 - 4}
              y2={centerY}
              stroke="rgba(var(--theme-spark-rgb), 0.5)"
              strokeWidth={1.6}
