...
def test_adventure_submit_rejects_forged_next_state_at_api_boundary(db, django_user_model):
    fixture = create_stage_readme_adventure_run(django_user_model, username="adventure-integrity")
    run = fixture.run
    ...
    assert not CommandStep.objects.filter(attempt=run).exists()
    assert not AdventureLevelCompletion.objects.filter(adventure_run=run).exists()


def test_adventure_submit_rejects_stale_client_revision_at_api_boundary(db, django_user_model):
    fixture = create_stage_readme_adventure_run(django_user_model, username="adventure-stale")
    run = fixture.run
...
def test_adventure_submit_accepts_verified_transition_and_completes(db, django_user_model):
    fixture = create_stage_readme_adventure_run(django_user_model, username="adventure-valid")
    run = fixture.run
    ...
    assert AdventureLevelCompletion.objects.filter(adventure_run=run).exists()
