...
def _solve_current(service, run, *, solved=True) -> bool:
    ...
    service.record_attempt_result(
        attempt=attempt,
        solved=solved,
        counted_command_count=attempt.adventure_level.min_counted_commands or 1,
        command_count=1,
        hint_count=0,
    )
    return True
...
def test_run_completes_when_all_commands_mastered(db, django_user_model):
    ...
    adventure = _adventure_with_levels()
...
def test_run_never_passes_while_a_command_is_unsolved(db, django_user_model):
    ...
    run = service.start_run(user=user, adventure=adventure)
    first_id = ordered_levels_for(adventure)[0].id
    ...
    for _ in range(200):
        ...
        service.record_attempt_result(
            attempt=attempt,
            solved=attempt.adventure_level_id != first_id,
...
def test_replay_after_pass_is_playable_but_uncounted(db, django_user_model):
    ...
    assert served == len(ordered_levels_for(adventure))
...
def test_adventure_run_http_flow_solves_a_level(db, django_user_model):
