...
from practice.services.builders import LevelVariantBuildError, StaticLevelVariantBuilder
...
def test_builder_validates_variant_lengths_before_bulk_persistence(db):
    suffix = unique_suffix()
    _, chapter = create_test_chapter(suffix=f"builder-length-{suffix}")
    challenge_level = ChallengeLevel.objects.create(
        chapter=chapter,
        slug=f"challenge-{suffix}",
        title="Variant Length Contract",
    )
    challenge_trial = ChallengeTrial.objects.create(
        challenge_level=challenge_level,
        difficulty=ChallengeTrial.Difficulty.EASY,
    ...
    # This is the longest currently authored case identifier. It exceeds
    # Django SlugField's implicit 50-character default and must remain valid.
    authored_case_id = "frost-survive-the-conflict-mastery-medium-transplant"
    template, case = _outcome_only_template(authored_case_id)
    variant = builder.build(level=challenge_trial, template=template, case=case, index=1)
    assert variant.slug == authored_case_id

    template, case = _outcome_only_template("x" * 161)
    with pytest.raises(LevelVariantBuildError, match="persisted model contract"):
        builder.build(level=challenge_trial, template=template, case=case, index=2)
