...
class MergeStashVerificationMixin:
    def _verify_stash(self, *, command: str, previous_state: dict, next_state: dict) -> None:
        ...
        if subcommand in {"list", "show"}:
            self._require_same_state(
                previous_state,
                next_state,
                f"git stash {subcommand} cannot mutate repository state.",
            )
            return
        ...
        if subcommand in {"push", "save"}:
            ...
            if (
                not working_tree
                and not (previous_state.get("staging") or {})
                and not (previous_state.get("conflicts") or [])
            ):
                self._require_same_state(
                    previous_state, next_state, "empty git stash cannot mutate repository state."
                )
                return
            message = (
                self._option_value(parts, "-m", "--message")
                or f"WIP on {self._head_branch(previous_state) or 'HEAD'}"
            )
            ...
            self._set_operation_metadata(
                expected,
                {
                    ...
                    "stash_count": len(expected.get("stash_stack") or []),
                },
            )
            ...
            entry = (
                (expected.get("stash_stack") or [])[real_index]
                if 0 <= real_index < len(expected.get("stash_stack") or [])
                else None
            )
            if not entry:
                raise BadRequest(
                    "execution.next_state does not match the submitted git stash command."
                )
            expected.setdefault("working_tree", {}).update(
                copy.deepcopy(entry.get("working_tree") or {})
            )
            ...
            self._set_operation_metadata(
                expected,
                {
                    "last_stash_action": subcommand,
                    "last_stash_operation": subcommand,
                    "stash_count": len(expected.get("stash_stack") or []),
                },
            )
    ...
    def _expected_merge_state(self, *, command: str, previous_state: dict) -> dict:
        ...
        if "--continue" in parts:
            ...
            if not previous_state.get("merge_parent") or (previous_state.get("conflicts") or []):
                raise BadRequest(
                    "execution.next_state does not match the submitted git merge --continue command."
                )
            expected = copy.deepcopy(previous_state)
            ...
            for path, value in target_tree.items():
                if value != current_tree.get(path):
                    expected_staging[path] = {
                        "status": self.normalizer.change_type(current_tree.get(path), value),
    ...
    def _verify_mergetool(self, *, command: str, previous_state: dict, next_state: dict) -> None:
        ...
        if not conflicts:
            raise BadRequest(
                "execution.next_state does not match the submitted git mergetool command."
            )
        ...
        tool = (
            self._option_value(parts, "--tool")
            or (previous_state.get("config") or {}).get("merge.tool")
            or (previous_state.get("operation_metadata") or {}).get("configured_merge_tool")
            or "tool"
        )
        expected_metadata = copy.deepcopy(previous_state.get("operation_metadata") or {})
        expected_metadata.update(
            {
                "last_mergetool_tool": tool,
    ...
    def _authored_conflict_paths(self, state: dict) -> list[str]:
        if not state.get("conflict_on_merge") and not state.get("merge_conflicts"):
            return []
        paths = list(state.get("conflict_files") or []) + list(
            state.get("merge_conflict_files") or []
        )
    ...
    def _overlapping_conflicts(
        self, base_tree: dict, current_tree: dict, target_tree: dict
    ) -> list[str]:
        conflicts: list[str] = []
    ...
    def _conflict_entry(
        self, branch: str, base: Any, ours: Any, theirs: Any, resolution: Any
    ) -> dict:
        return {
            "status": "conflicted",
            "content": f"<<<<<<< HEAD\n{ours or ''}\n=======\n{theirs or ''}\n>>>>>>> {branch}\n",
