...
class CommitHistoryVerificationMixin:
    ...
    def _verify_commit(self, *, command: str, previous_state: dict, next_state: dict) -> None:
        ...
        if "-a" in parts or "--all" in parts:
            staged_by_all = self._selected_add_paths(
                expected, [], include_tracked=True, include_untracked=False
            )
            self._stage_selected(expected, staged_by_all)

        if "--amend" in parts:
            self._verify_commit_amend(
                parts=parts,
                previous_state=previous_state,
                staged_state=expected,
        ...
        merge_branch = str(
            (expected.get("operation_metadata") or {}).get("last_merge_branch") or "branch"
        )
        ...
        if len(next_commits) != len(previous_commits) + 1:
            raise BadRequest(
                "execution.next_state does not match the submitted git commit command."
            )
        self._require_equivalent_expected(expected, next_state, "git commit")

    def _verify_commit_amend(
        self, *, parts: list[str], previous_state: dict, staged_state: dict, next_state: dict
    ) -> None:
        current = self.normalizer.head_commit_id(staged_state)
        old_commit = self.normalizer.commit_by_id(staged_state, current)
        if not old_commit:
            raise BadRequest(
                "execution.next_state does not match the submitted git commit --amend command."
            )
        ...
        if not staged_entries and self._commit_message(parts) is None and "--no-edit" not in parts:
            self._require_same_state(
                previous_state,
                next_state,
                "git commit --amend without changes cannot mutate repository state.",
            )
            return
        ...
        self._set_operation_metadata(
            expected,
            {"last_amend_replaced_commit": current, "last_amend_created_commit": commit_id},
        )
        expected["staging"] = {}
            ...
            expected.setdefault("reflog", []).append(
                {
                    ...
                    "target": commit_id,
                    "message": f"commit --amend: replaced {current}",
                }
            )
        expected = self.tools.normalize_state(expected)
    ...
    def _verify_rebase(self, *, command: str, previous_state: dict, next_state: dict) -> None:
        ...
        if "--abort" in parts:
            rebase_state = (
                previous_state.get("rebase_state")
                if isinstance(previous_state.get("rebase_state"), dict)
                else None
            )
            abort_state = (rebase_state or {}).get("abort_state")
            if not isinstance(abort_state, dict):
                raise BadRequest(
                    "execution.next_state does not match the submitted git rebase --abort command."
                )
        ...
        if "--continue" in parts:
            if not isinstance(previous_state.get("rebase_state"), dict) or (
                previous_state.get("conflicts") or []
            ):
                raise BadRequest(
                    "execution.next_state does not match the submitted git rebase --continue command."
                )
            remaining = [
                str(item)
                for item in ((previous_state.get("rebase_state") or {}).get("remaining") or [])
            ]
            if not remaining:
                expected = copy.deepcopy(previous_state)
                self._set_operation_metadata(
                    expected,
                    {
                        ...
                        "last_rebase_replayed_commits": copy.deepcopy(
                            (expected.get("rebase_state") or {}).get("applied") or []
                        ),
                    },
                )
        ...
        if not args:
            raise BadRequest(
                "execution.next_state does not match the submitted git rebase command."
            )
        ...
        if branch_ref:
            ...
            expected["head"] = {
                "type": "branch",
                "name": branch_ref,
                "target": (expected.get("branches") or {}).get(branch_ref),
            }
        ...
        if branch_tip == new_base:
            self._require_same_state(
                previous_state, next_state, "up-to-date git rebase cannot mutate repository state."
            )
            return
        applied = self._replay_linear_commits(
            expected, branch_tip, self._common_ancestor(expected, branch_tip, upstream), new_base
        )
        self._set_operation_metadata(
            expected,
            {
    ...
    def _verify_revert(self, *, command: str, previous_state: dict, next_state: dict) -> None:
        ...
        if not source:
            raise BadRequest(
                "execution.next_state does not match the submitted git revert command."
            )
        ...
        for path, change in (source.get("changes") or {}).items():
            reverted_changes[path] = {
                "change_type": self.normalizer.change_type(
                    head_tree.get(path), change.get("before")
                ),
        ...
        self._set_operation_metadata(
            expected,
            {
                ...
                "last_revert_no_edit": "--no-edit" in parts,
            },
        )
        expected.setdefault("reflog", []).append(
            {
    ...
    def _verify_cherry_pick(self, *, command: str, previous_state: dict, next_state: dict) -> None:
        ...
        if not source:
            raise BadRequest(
                "execution.next_state does not match the submitted git cherry-pick command."
            )
        head_id = self.normalizer.head_commit_id(previous_state)
        head_tree = self.normalizer.head_tree(previous_state)
        ...
        if "--no-commit" in parts or "-n" in parts:
            expected["staging"] = {
                ...
                if change.get("after") is None
                    ...
                    "status": change.get("change_type"),
                    "content": copy.deepcopy(change.get("after")),
                }
    ...
    def _replay_linear_commits(
        self, state: dict, branch_tip: str | None, stop_at: str | None, new_base: str | None
    ) -> list[str]:
    ...
    def _common_ancestor(self, state: dict, left: str | None, right: str | None) -> str | None:
        ...
        return next(
            (commit_id for commit_id in self._history(state, left) if commit_id in right_history),
            None,
        )
