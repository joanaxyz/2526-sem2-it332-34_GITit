...
class RefVerificationMixin:
    def _verify_branch(self, *, command: str, previous_state: dict, next_state: dict) -> None:
        ...
        if "-m" in parts or "--move" in parts:
            ...
            if (
                not old_name
                or not new_name
                or old_name not in expected_branches
                or new_name in expected_branches
            ):
                raise BadRequest(
                    "execution.next_state does not match the submitted git branch command."
                )
            ...
            if expected_head.get("type") == "branch" and expected_head.get("name") == old_name:
                expected_head = {
                    "type": "branch",
                    "name": new_name,
                    "target": expected_branches.get(new_name),
                }
            if old_name in expected_upstream:
                expected_upstream[new_name] = expected_upstream.pop(old_name)
            if (
                next_state.get("branches") != expected_branches
                or next_state.get("head") != expected_head
                or next_state.get("upstream_tracking") != expected_upstream
            ):
                raise BadRequest(
                    "execution.next_state does not match the submitted git branch command."
                )
        ...
        if "-d" in parts or "-D" in parts or "--delete" in parts:
            ...
            if "-D" not in parts and not self._is_ancestor(
                previous_state, target, self.normalizer.head_commit_id(previous_state)
            ):
                raise BadRequest(
                    "execution.next_state does not match the submitted git branch command."
                )
        ...
        target = (
            self._resolve_ref(previous_state, start)
            if start
            else self.normalizer.head_commit_id(previous_state)
        )
    ...
    def _verify_switch(self, *, command: str, previous_state: dict, next_state: dict) -> None:
        ...
        if "--detach" in parts:
            ...
            if not target:
                raise BadRequest(
                    "execution.next_state does not match the submitted git switch command."
                )
    ...
    def _verify_checkout(self, *, command: str, previous_state: dict, next_state: dict) -> None:
        ...
        if "-b" in parts:
            ...
            if start and not target:
                raise BadRequest(
                    "execution.next_state does not match the submitted git checkout command."
                )
            expected_branches = copy.deepcopy(previous_state.get("branches") or {})
            expected_branches[name] = target
            if next_state.get("branches") != expected_branches:
    ...
    def _verify_tag(self, *, command: str, previous_state: dict, next_state: dict) -> None:
        ...
        if "-d" in parts or "--delete" in parts:
            ...
            self._require_same_state(
                previous_state,
                next_state,
                "git tag without arguments cannot mutate repository state.",
            )
            return
        else:
            name = args[0]
            ...
            if not target:
                raise BadRequest(
                    "execution.next_state does not match the submitted git tag command."
                )
    ...
    def _verify_config(self, *, command: str, previous_state: dict, next_state: dict) -> None:
        ...
        if "--list" in parts or "-l" in parts:
            self._require_same_state(
                previous_state, next_state, "git config --list cannot mutate repository state."
            )
            return
        for key in {
            "repository_initialized",
        ...
        if len(args) < 2:
            raise BadRequest(
                "execution.next_state does not match the submitted git config command."
            )
    ...
    def _resolve_revision(self, state: dict, revision: str | None) -> str | None:
        ...
        if revision.startswith("HEAD@{") and revision.endswith("}"):
            try:
                ...
                return None
            entry = (
                (state.get("reflog") or [None])[index]
                if index < len(state.get("reflog") or [])
                else None
            )
