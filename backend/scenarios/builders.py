...
from scenarios.models import CommandDrill, ProblemVariant, WorkflowScenarioLevel
...
class StaticProblemVariantBuilder:
    ...
    def build(self, *, problem, template: dict[str, Any], case: dict[str, Any], index: int) -> ProblemVariant:
        ...
        variant = ProblemVariant(
            command_drill=problem if isinstance(problem, CommandDrill) else None,
            workflow_level=problem if isinstance(problem, WorkflowScenarioLevel) else None,
    ...
    def validate(self, variant: ProblemVariant) -> None:
