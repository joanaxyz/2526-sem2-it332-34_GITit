...
def _syntax_how_to_read(command: str) -> list[str]:
    ...
    if "--quiet" in normalized or " -q" in f" {normalized}":
        items.append(
            "Quiet commands can succeed with no visible output, so verification must come from status, metadata, or the target state rather than terminal chatter."
        )
    if "<directory>" in command:
        items.append(
            "The directory argument matters because initializing or cloning the parent folder is a different answer from targeting the named child folder."
        )
    if "<branch>" in command:
        items.append(
            "The branch option changes which branch name is created or checked out; it is not just decoration."
        )
    if "<path>" in command:
        items.append(
            "The path limits the operation. A broad command can accidentally stage, restore, or inspect files that the scenario wanted left alone."
        )
    if "<number>" in command:
        items.append(
            "The number limits history depth or output count, so use the exact amount requested."
        )
    if '"message"' in command:
        items.append(
            "The message text must match the scenario's required commit message, not the sample word shown here."
        )
...
def _syntax_when_to_use(command: str) -> list[str]:
    ...
    if normalized.startswith("git init"):
        items.append(
            "Use this during repository setup, before any first commit exists unless the task explicitly says you are safely reinitializing."
        )
    elif normalized.startswith("git clone"):
        items.append(
            "Use this when the required starting point is a remote repository, not a local folder that already exists."
        )
    elif normalized.startswith("git add"):
        items.append(
            "Use this after inspecting changes and before committing, when the chosen content belongs in the next snapshot."
        )
    elif normalized.startswith("git commit"):
        items.append(
            "Use this only after the index contains exactly the snapshot that should be saved."
        )
    elif normalized.startswith("git restore --staged"):
        items.append(
            "Use this when content is staged but should be kept as an unstaged working-tree change."
        )
    elif normalized.startswith("git restore"):
        items.append(
            "Use this when the scenario says selected working-tree edits should be discarded."
        )
    elif normalized.startswith("git rm"):
        items.append(
            "Use this when a tracked generated file should stop being tracked but remain on disk."
        )
    elif normalized == ".gitignore":
        items.append(
            "Use this when the scenario asks you to ignore generated, local, or secret files before staging the ignore rule."
        )
    else:
        items.append(
            "Use this as an inspection step when the scenario asks you to read repository evidence before acting."
        )
    items.append(
        "Do not choose it just because it looks familiar; match it to the scenario's exact required values."
    )
    return _clean_items(items)
...
def _placeholder_note(command: str) -> str:
    ...
    return (
        "Choose this syntax only when its option, path, or argument matches the scenario details."
    )


def _verification_items(
    *, command: str, effects: list[str], boundaries: list[str], readiness: list[str]
) -> list[str]:
    items = []
    if effects:
    ...
    if command.startswith("git status"):
        items.append(
            "Verify the branch line and each path category: staged, unstaged, untracked, and ignored when requested."
        )
    elif command.startswith("git diff --staged") or command.startswith("git diff --cached"):
        items.append(
            "Verify that the displayed patch is the version already staged for the next commit."
        )
    elif command.startswith("git diff"):
        items.append(
            "Verify whether you are reading unstaged edits, staged edits, or all tracked edits since HEAD."
        )
        ...
        items.append(
            "Run a status or staged diff afterward to confirm only the intended paths or hunks entered the index."
        )
    elif command.startswith("git commit"):
        items.append(
            "Run a log or status check afterward to confirm the new or amended snapshot is the intended one."
        )
    elif command.startswith("git restore"):
        items.append(
            "Run status afterward because restore affects either the working tree or index depending on the flags used."
        )
    elif command.startswith("git init"):
        items.append(
            "Run status afterward to confirm the folder is now a repository without assuming a commit was created."
        )
    elif command.startswith("git clone"):
        items.append(
            "Inspect status and remotes afterward to confirm the local copy is clean and connected to origin."
        )
    elif command == ".gitignore":
        items.append(
            "Pair the ignore rule with status or check-ignore so you can prove the generated path is excluded."
        )
    return _clean_items(items)
...
def _state_language(*, command: str, effects: list[str], boundaries: list[str]) -> str:
    ...
    return _first_sentence(
        effects[0] if effects else "Use this command only when it matches the scenario state."
    )
