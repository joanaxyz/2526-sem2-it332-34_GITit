...
# The command library is seeded content, not baked in. Authoring lives in the
# ``seed_command_library`` management command (and, once a storage model exists,
# the database). This list stays empty until that seed is wired; the builder
# helpers above (``_content``, ``_sections``, ``_diagram`` …) are what the seed
# uses to turn authored fields into renderable pages.
GIT_COMMAND_CONTENT_LIBRARY: list[dict[str, Any]] = []


# Index the library so the Storey Book can resolve a registered command to its
# content in O(1). Keyed by the entry ``key`` and the normalized canonical
# command, so lookups work from either a ``base_command`` or a written command.
def _build_command_content_index() -> dict[str, dict[str, Any]]:
    index: dict[str, dict[str, Any]] = {}
    for entry in GIT_COMMAND_CONTENT_LIBRARY:
        index[entry["key"]] = entry
        index.setdefault(normalize_command_text(entry["canonical_command"]), entry)
    return index


_COMMAND_CONTENT_INDEX = _build_command_content_index()


def command_content_entry_for_command(command: str) -> dict[str, Any] | None:
    """Resolve a registered command (``git status``, ``git log --oneline``, …) to
    its authored library entry, or ``None`` when nothing is authored for it."""
    if not command:
        return None
    key = command_content_key_for_command(command)
    entry = _COMMAND_CONTENT_INDEX.get(key)
    if entry is not None:
        return entry
    return _COMMAND_CONTENT_INDEX.get(normalize_command_text(command))
