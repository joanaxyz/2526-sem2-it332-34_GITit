...
def render(raw: str, normalized: str, display_command: str, prompt_text: str) -> str | None:
    ...
    if normalized.startswith("git stash list"):
        return transcript(
            display_command,
            "stash@{0}: WIP on feature/map: 9c2e6b1 Add map encounter preview\nstash@{1}: WIP on main: 51f2a90 Document chapter unlocks",
        )
    ...
    if normalized.startswith("git stash drop"):
        return transcript(
            display_command, "Dropped refs/stash@{0} (8f3c2d1a4b5c6d7e8f90123456789abcdef0123)"
        )

    if normalized.startswith("git stash"):
        return transcript(
            display_command,
            "Saved working directory and index state WIP on feature/map: 9c2e6b1 Add map encounter preview",
        )
    ...
    if normalized.startswith("git remote -v"):
        return transcript(
            display_command,
            "origin\thttps://example.com/arcane-spire.git (fetch)\norigin\thttps://example.com/arcane-spire.git (push)",
        )
    ...
    if normalized.startswith("git push") and "--delete" in normalized:
        return transcript(
            display_command,
            "To https://example.com/arcane-spire.git\n - [deleted]         feature/old-map",
        )
