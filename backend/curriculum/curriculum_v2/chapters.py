"""Chapter plan for the v2 Git-it curriculum.

The tower is ordered by concept dependency. A Challenge in a chapter may combine
commands from that chapter's Command Adventure and earlier Command Adventures,
...
def relic_layout() -> dict:
    """Per-chapter relic-layout overrides for the official Archive.

    Relics are positioned by ``curriculum.selectors.relic_layout_payload`` from
    sensible defaults; a chapter may later override positions/assets here. Empty
    for now (the single official relic art is used everywhere)."""
    return {}
