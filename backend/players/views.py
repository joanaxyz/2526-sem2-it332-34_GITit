...
from shop.serializers import ShopEquipResponseSerializer, ShopMutationRequestSerializer
...
class PlayerPreferencesAPIView(APIView):
    ...
    @extend_schema(
        request=PlayerPreferencesSerializer, responses={200: PlayerPreferencesSerializer}
    )
...
class CompanionLoadoutAPIView(APIView):
    @extend_schema(
        request=ShopMutationRequestSerializer, responses={200: ShopEquipResponseSerializer}
    )
    def post(self, request):
        ...
        response = ShopEquipResponseSerializer({**result, "shop": shop_payload(player=player)})
