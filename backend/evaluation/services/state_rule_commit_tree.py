...
def check_commit_tree_state_rule(
    ...
    *,
    ...
    if rule_type == "ignored_paths_present":
        paths = set(as_list(rule.get("path") or rule.get("paths")))
        allowed_statuses = set(as_list(rule.get("statuses") or ["ignored", "untracked"]))
    ...
    if rule_type == "gitignore_matches_paths":
        ...
        paths = set(as_list(rule.get("paths") or rule.get("ignored_paths")))
        missing = sorted(path for path in paths if not matching_gitignore_rule(content, path))
        return (
            not missing,
