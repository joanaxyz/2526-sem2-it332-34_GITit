...
from assets.models import KIND_CHARACTER, KIND_TOWER_PIECE, Asset, TowerPieceAsset
...
def _kit_piece(slug="official-spire", piece_type="spire"):
    asset = Asset.objects.create(
        kind=KIND_TOWER_PIECE, slug=slug, label="Arcane Spire", is_published=True, tags=[DEFAULT_KIT_TAG]
    )
    TowerPieceAsset.objects.create(asset=asset, piece_type=piece_type)
    return asset
...
def test_default_kit_lists_only_tagged_official_assets():
    ...
    Asset.objects.create(kind=KIND_TOWER_PIECE, slug="plain", label="Plain", is_published=True, tags=[])
...
def test_registration_grants_the_kit(django_user_model):
    ...
    assert Entitlement.objects.filter(user=user, asset__slug="official-spire").exists()
...
def test_granted_official_piece_stays_labelled_official(django_user_model):
    ...
    descriptors = owned_descriptor_map(user, KIND_TOWER_PIECE)
    assert descriptors["official-spire"]["source"] == "official"
