"""Phase 4: authored chapter settings (reward checkpoints, rosters, pass bar)
reach the compiled runtime chapter/adventure instead of being hard-coded."""
...
def test_admin_authored_chest_rewards_and_pass_bar_reach_runtime(django_user_model):
    ...
    assert runtime.chapter.chest_rewards == [
...
@pytest.mark.django_db
def test_custom_tower_chapter_grants_no_coins(django_user_model):
...
def test_missing_chest_rewards_fall_back_to_defaults(django_user_model):
    ...
    assert len(runtime.chapter.chest_rewards) == 4


@pytest.mark.django_db
def test_chapter_groups_content_into_one_runtime_chapter_with_its_rewards(django_user_model):
    user = make_admin(django_user_model)
    chapter = AuthoringChapter.objects.create(
        owner=user,
        slug="floor-1",
    ...
    adventure = ContentDefinition.objects.create(
        owner=user, kind="adventure", chapter=chapter, slug="adv", title="Adv",
        command_family="git status", definition=_playable_definition(),
    )
    challenge_a = ContentDefinition.objects.create(
        owner=user, kind="challenge", chapter=chapter, slug="cha", title="Cha A",
        command_family="git status", definition=_playable_definition(),
    )
    challenge_b = ContentDefinition.objects.create(
        owner=user, kind="challenge", chapter=chapter, slug="chb", title="Cha B",
        command_family="git status", definition=_playable_definition(),
    )
    ...
    # A chapter holds 1 adventure + 1+ challenges, all sharing ONE runtime chapter.
    ...
    # Both challenges live under the shared chapter.
...
def test_authored_battle_stage_reaches_runtime(django_user_model):
    # The chapter's authored battle-stage dressing compiles onto the runtime
    # Chapter so the live battle can render it.
    user = make_user(django_user_model)
    stage = {
    ...
    content = ContentDefinition.objects.create(
        owner=user, kind="adventure", chapter=chapter, slug="adv-stage", title="Adv",
        command_family="git status", definition=_playable_definition(),
    )

    runtime = ContentRuntimeCompiler().compile(content=content)

    assert runtime.chapter.battle_stage == stage
