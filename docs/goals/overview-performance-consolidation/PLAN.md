...
**Acceptance evidence:** Typed Overview/Admin/routing tests; unchanged player performance contract proof; new staff analytics payload and non-staff denial proof; a generated-contract three-way audit proving preservation of the existing onboarding enum delta plus only the intended Admin analytics schema delta; screenshots at 1440×900, 820×900, and 390×844 for learner SCR/HLCR/empty states and desktop/mobile Admin analytics; keyboard traces for the learner radiogroup and Admin navigation/table; proof that learner selection issues zero additional requests; exactly one player performance request on initial Home; `/performance` redirect proof; CAR/RTR/ARC absent from learner UI and present in Admin; zero-vs-null assertions; no horizontal overflow; clean console/page errors; production build, focused/full tests, lint, regenerated API checks, architecture guard, and typography check.
...
| `arc` | Average retries | Technical effort/cost diagnostic | Lower means fewer retries; do not score it as mastery | Admin only; overall + Modules 1–4 | Exact scalar values with relative module bars scaled to the largest non-null displayed module value; label the scale “relative to this set,” never as a target or score |

Charts must remain semantic without color: each row includes module name, exact value, and evidence text. Color is one restrained product accent; null/empty uses muted treatment. The learner uses a two-option `radiogroup` because only two choices remain; Arrow keys choose between visible radios, and the resulting `metric` URL parameter is shareable. No chart library is needed.
...
**Verification:** Component tests for default/each selection, invalid query fallback, parameter preservation, `radiogroup` Arrow-key semantics and Tab exit, percentage scaling, explicit absence of CAR/RTR/ARC, four module rows, zero/null distinction, and input immutability.
