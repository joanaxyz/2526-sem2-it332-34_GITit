# Pre-Slice-7 Dashboard/Home Summary Contract Baseline

Captured on 2026-08-10 after the amended PRE review returned `ALIGNED` with no findings and before any Slice 7 production, test, generated-contract, fixture, or shared-guard implementation edit.

## Preservation Rule

The 103-entry dirty manifest belongs to completed Slice 1-6 work plus the approved Slice 7 plan package. At the terminal gate, 98 entries must remain byte-identical. The other five pre-existing dirty targets may change only for the planned Dashboard cutover:

- `backend/progress/serializers.py` — exact Dashboard serializer replacement; existing RateMetric/Stats contracts unchanged;
- generated `openapi.json` and `apiTypes.ts` — generator-only Dashboard projection changes; existing Stats semantics unchanged;
- the shared architecture checker and algorithm tests — additions only; every earlier rule/test remains.

Clean Dashboard frontend targets are separately hashed below and may change only as planned. New `PRE_SLICE_BASELINE.md`, `EVIDENCE.md`, and the new Dashboard endpoint test are outside the captured manifest. The two Home feature shims are protected and must remain byte-identical.
...
```text
 ...
 M | 193547 | C8FAC3FBD58A1C71FDA740023E4C7D06D75902CC04243322C74D41B6476B7023 | frontend/src/shared/api/generated/openapi.json
 M | 2094 | 1CAC0B9BF114E1904562455BEAF7606B3D36C644147EA83DF6F62796292B1E29 | backend/progress/serializers.py
 ...
 M | 33995 | 832E179F812CCBC689393589051737F2EB2BF55716C5EFA57309291FF55DBA41 | backend/common/tests/test_architecture_guard_algorithms.py
 ...
 M | 43184 | F7EBA399021E3C7A88EDCAE08703B80C97689DB8344DC0D3C3A7883544C8C643 | frontend/src/shared/api/generated/apiTypes.ts
 ...
 M | 82693 | F289F899EEA00CB12619AC7D3BF8AD131CB61BDEBBA436B09688E7D770744F0A | scripts/checks/check_architecture_boundaries.py
...
Manifest entries: 103. Terminal strict preservation target: 98 of 98 byte-identical; five shared dirty targets are reviewed separately.

## Shared and Clean Target Baseline

| Path | Lines | Bytes | SHA-256 | Existing numstat |
|---|---:|---:|---|---|
| `backend/progress/serializers.py` | 59 | 2,094 | `1CAC0B9BF114E1904562455BEAF7606B3D36C644147EA83DF6F62796292B1E29` | `52 + / 2 -` |
| `frontend/src/shared/api/generated/openapi.json` | 5,555 | 193,547 | `C8FAC3FBD58A1C71FDA740023E4C7D06D75902CC04243322C74D41B6476B7023` | `141 + / 14 -` |
| `frontend/src/shared/api/generated/apiTypes.ts` | 492 | 43,184 | `F7EBA399021E3C7A88EDCAE08703B80C97689DB8344DC0D3C3A7883544C8C643` | `6 + / 1 -` |
| `frontend/src/shared/progress/types.ts` | 38 | 825 | `7FF64338D5AA7A35A911DD68C6B3E07D0669ECA0F63A930F54C7C2D71044FC72` | clean |
| `frontend/src/shared/progress/homeSummaryApi.ts` | 12 | 572 | `496C44EF1DE1193FE5F69CB9C92BA1EB8CF6B8564F9E8EC02DF74777BF0DE102` | clean |
| `frontend/src/features/home/preview/fixtures.ts` | 83 | 3,302 | `C81AF5304BF24DAEADF49F68BA80A2AB728912E5E3D0EB39F3C24E40606C0DBA` | clean |
| `scripts/checks/check_architecture_boundaries.py` | 2,201 | 82,693 | `F289F899EEA00CB12619AC7D3BF8AD131CB61BDEBBA436B09688E7D770744F0A` | `1762 + / 2 -` |
| `backend/common/tests/test_architecture_guard_algorithms.py` | 798 | 33,995 | `832E179F812CCBC689393589051737F2EB2BF55716C5EFA57309291FF55DBA41` | `778 + / 0 -` |

The two guard files may only increase additions; their deletion counts must remain `2` and `0`. All existing 21 algorithm tests and live rules must remain. Serializer/generated targets must preserve completed Stats semantics exactly.

## Protected Baseline

| Path | Lines | Bytes | SHA-256 |
|---|---:|---:|---|
| `backend/progress/services/metrics.py` | 442 | 18,930 | `BC10D8BFADC22F33AB0A4DE0D1692F92C49E3628F94AD2217EBD0FA843EEA38F` |
| `backend/progress/views.py` | 33 | 1,222 | `B47DFA61F7EB58C5D4D7BB1EFB4A862B041631C723FEFC3A7DC1D303A1EE648F` |
| `backend/progress/urls.py` | 9 | 377 | `A9A7D87771E85D4AB917A621D55DBD800FE687A53644BF28154AF89FE25C8F97` |
| `backend/common/openapi.py` | 204 | 8,148 | `14DD17A24B0C7A0436164CCB3A27816017FE0A1843690D83BD6CD5505933F8A5` |
| `scripts/api/api_contract.py` | 464 | 19,497 | `FCF41D695D712401E5F98BC347995E89079E3DDCF132DAC6C29FBD1618B14D23` |
| `frontend/src/shared/api/httpClient.ts` | 170 | 5,433 | `41B94C89B7E713598002AD1D0FC627CA63CC13171338D27802A12F4DA59DF8C5` |
| `frontend/src/features/home/types.ts` | 1 | 62 | `B2CA6CAFE5F24B4E5745B14801A642EC6702FA0DAE5F352C2608BF13F238871C` |
| `frontend/src/features/home/api/homeApi.ts` | 1 | 77 | `8140C752589F6E6CEEDE2AE186FF32C351A0E2B417D4B57B17C097F9C7394AF4` |
| `backend/progress/tests/test_stats_summary_api.py` | 124 | 3,898 | `E8EF52C4686AC62FD9A65DCCC252A65826198A758B35B95B62C36DEC26A75EDE` |
| `frontend/src/features/stats/types.ts` | 5 | 255 | `1AB6D119FC5E3A31D1B9492DA4803776B2220B8689EF963643E596F22EDF70BB` |
| `frontend/src/features/stats/api/statsApi.ts` | 7 | 188 | `03032F98394096617A57834BADCEB6348BE42F54E80BBAA360270040A058DD94` |

Exact protected Home shims:
...
```ts
export { homeSummaryApi as homeApi } from '@/shared/progress/homeSummaryApi'
```

## Authenticated Runtime Baseline

Fresh migrated in-memory databases and authenticated real URL/view/service requests proved:

- empty account: HTTP 200; exact nine top-level keys; exact `scr`/`arc`/`hlcr` rate objects; empty dynamic chapter map; counts `0/0/0/0`; both completion keys present with `null`/`[]`; nullable dated streak; zero perfect clears/mastery; `retry_trends` is `list` and empty;
- seeded account with two real Challenge runs: counts `2/1/1/0`; top-level and chapter `scr.value` are `50.0`; chapter key `920001` maps to exact KPI fields; streak is `3/5/2026-08-09`; `retry_trends` is a one-row list with `{level_title: "Stage README Challenge", attempts: 2, retries: 1, label: "1 retry runs"}`.

These runtime values are the executable contract to document, not change.

## Declared/Generated Mismatch Baseline

The current Dashboard serializer/OpenAPI/generated TypeScript:

- use loose dictionaries for `kpis`, `chapter_kpis`, `counts`, and `streak`;
- declare `retry_trends` as an object/dictionary although runtime returns a list;
- mark `completed_story_slug` optional although runtime always emits the key;
- generate `DashboardSummaryResponse.retry_trends` as `{ [key: string]: JsonValue }`;
- allow the handwritten `HomeSummary` DTO and `HomeSummaryResult = ApiSchemas['DashboardSummaryResponse'] & HomeSummary` override to mask the mismatch.

## Passing-but-Insufficient Gates

Before cutover, the API-current, frontend API-usage/type-adoption, live architecture, CSS, full frontend, and existing Progress/Stats tests pass. They prove internal generated consistency and current consumer behavior, but none proves the real Dashboard response matches the declared schema. Slice 7 must close that gap without changing runtime or Home behavior.
