...
The platform serves third-year BSIT students at CIT-U enrolled in team-based development courses, including Information Management 2 and Applications Development. The current release uses a static, pre-authored Git starter syllabus seeded before deployment, while syllabus and content configuration is reserved for the planned Phase 2 Administrative Management module. The starter content library comprises Unit 1 Orientation lessons with non-scorable interactive concept walkthroughs, published learning units, unit-level Lesson Overview pages, scenario skill focuses, difficulty instances, authored command-count policies, and RTA-eligible retry variants. A Lesson Overview is a single scrollable instructional page for that learning unit; it may contain explanations, examples, callouts, command snippets, diagrams, or DAG illustrations as needed, but it is not constrained to a fixed static template. The exact scenario count, learning-unit names, difficulty-instance coverage, and variant library size are finalized as implementation content before deployment rather than fixed as permanent SRS commitments, allowing the team to adjust content coverage before pilot release if syllabus needs change. Students can access the seeded content but cannot create or modify it in the current release.

The student-facing interface includes the following core components: registration and login screens, Student Dashboard, Dashboard/Units tab, Learning Unit list, expandable unit cards with scenario lists, Unit 1 Orientation interface, Lesson Overview page, View Scenarios panel, Scenario Selection interface, Difficulty Selection/Unlocking interface, Scenario Practice Workspace, terminal-style command input, Live DAG panel, Expected-State Diagram panel, Contextual Feedback panel, Review Mode, and student-facing progress/KPI indicators. Scenario selection may begin from two student-facing entry points: (1) the Lesson Overview page, where the student clicks **View Scenarios** after reading the unit overview, and (2) the Dashboard/Units tab, where the student expands a unit card and selects a scenario from the list displayed below that unit. Administrative Management, AI-assisted authoring, and administrator-facing content editing interfaces are planned Phase 2 / next-semester modules, not current-release requirements.

The system incorporates a light gamification layer — including a streak counter, first-attempt star indicators, and retry-count trend displays — to support sustained engagement. User authentication is handled via manual email/password registration and login with JWT-based session management. The system is accessible via modern web browser on desktop, tablet, and mobile form factors. No native mobile application is in scope.
...
The planned Administrative Management module (Module 6) is intended for the authorized platform administrator. It consolidates non-student capabilities into one area: viewing cohort-level and student-level KPI summaries derived from system logs, managing student accounts and access, creating and maintaining learning content units, maintaining Lesson Overview HTML/CSS content, adding or editing scenario records, managing scenario variants, and reviewing administrative audit logs. The current release still logs the data needed to compute the required student and cohort KPIs, while the administrative interface for viewing or maintaining those records is introduced only through Phase 2.

The planned AI-Assisted Learning Support module (Module 7) includes a conceptual Git chatbot, an AI-assisted Lesson Overview HTML/CSS drafting workflow, and an AI-assisted scenario-drafting workflow. AI-generated content requires administrator review, validation, and publication before becoming student-accessible.
...
| Live Animated DAG | The real-time, animated commit graph rendered on the GIT it! interface; updates after every command execution at all difficulty levels |
| Lesson Overview | A single scrollable instructional page rendered before scenario practice for a learning unit. Stored and displayed as administrator-approved or pre-authored HTML with scoped CSS. May include text, examples, callouts, command snippets, diagrams, or DAG illustrations. |
| View Scenarios Panel | The scenario-list section for a selected learning unit. May be opened from the Lesson Overview page through the **View Scenarios** action or from the Dashboard/Units tab by expanding a unit card. Lists the unit's scenario skill focuses and their Easy, Medium, and Hard access states. |
| Unit Card Expansion | The Dashboard/Units tab interaction where a student expands a learning unit card to display the scenarios belonging to that unit without first opening the Lesson Overview page |
...
| Scenario Skill Focus | The Git problem pattern targeted by a scenario, such as tracking untracked files, moving work to a branch, resolving a merge conflict, or recovering from an accidental reset |
| Scenario Instance | One specific difficulty-level version (Easy, Medium, or Hard) of a scenario |
| Scenario Variant | A structurally distinct template-based variation of a scenario skill focus, differing in branch names, commit structures, file-state markers, or starting repository state |
...
| OCG | Orientation Completion Gate — proportion of students who completed all Unit 1 orientation lessons before starting their first scenario session |
...
| RTA | Retry Transfer Accuracy — first-attempt completion rate on structurally changed retry variants served after a prior Failed or Abandoned scenario session for the same scenario skill focus; first-ever encounters are excluded |
...
GIT it! is a self-contained web-based Git learning platform that provides scenario-driven practice through a client-server architecture. Students access the system through a browser, complete Unit 1 orientation lessons, view learning-unit lesson overviews, select scenario skill focuses, choose available difficulty levels, enter simulated Git commands, and observe repository-state changes through a live DAG visualization.
...
- Students are assumed to have a valid email address for manual registration.
- The approved starter content library — including learning units, Lesson Overview pages, scenario skill focuses, difficulty-level instances, authored command-count policies, and required variant pools — is authored and seeded into the database prior to deployment. This syllabus/content set is static in the current release. The exact number, names, ordering, and syllabus coverage of learning units, scenario skill focuses, difficulty-level instances, and variants may be finalized before deployment and are not permanent SRS commitments.
- Unit 1 orientation content (animated DAGs, interactive click-through steps) is authored and deployed before the evaluation period begins.
- Scenario selection is reachable from two current-release UI paths: the Lesson Overview page through a **View Scenarios** action and the Dashboard/Units tab through expandable unit cards. Both paths resolve to the same scenario skill focus, difficulty access rules, variant-selection rules, and scenario-instance loading behavior.
- The Retry Transfer Accuracy (RTA) metric is computed only from RTA-eligible changed-variant retry sessions: sessions served after a prior Failed or Abandoned scenario session for the same scenario skill focus, where the new variant is structurally different from the prior attempt. A student's first-ever session for a given scenario skill focus is excluded from this metric.
...
|-----------|-------------|
| Frontend SPA (React.js / Vite) | Browser-based student interface for registration, login, dashboard, Units view, Lesson Overview pages, scenario selection, Scenario Practice Workspace, terminal input, live DAG visualization, expected-state diagram, contextual feedback panel, Review Mode, and progress indicators. Communicates with the backend exclusively through RESTful HTTPS endpoints using JSON payloads. |
...
| Supabase (PostgreSQL) | Managed relational database for persistent storage of user accounts, unit and lesson records, scenario definitions, variant pools, session logs, step logs, completion records, Review Mode attempt logs, and progress records. All writes involving completion records use database transactions and unique constraints to prevent duplicate entries. |
...
| **Alternative Flows** | AF1 — Email already registered: System returns HTTP 409; frontend displays 'An account with this email already exists. Please log in.'<br>AF2 — Client-side validation failure: Frontend displays inline field-level error messages before submission.<br>AF3 — Server-side validation failure: System returns HTTP 422 with field-level error details. |
| **Functional Requirements** | FR-AUTH-01: Validate email format, password complexity, and password confirmation match before creating an account.<br>FR-AUTH-02: Reject registration if the email is already associated with an existing account (HTTP 409).<br>FR-AUTH-03: Hash all passwords using a secure one-way password hashing algorithm before storage; plaintext passwords shall never be persisted or logged.<br>FR-AUTH-04: Upon successful registration, create an initial progress record for the new student.<br>FR-AUTH-05: Upon successful registration, authenticate the user and redirect to the Student Dashboard. |
...
| **Main Flow** | 1. Student navigates to /login.<br>2. System displays the login form: email address, password.<br>3. Student submits credentials.<br>4. System looks up the account by email.<br>5. System verifies the submitted password against the stored hash.<br>6. System generates an access token (15-minute expiry) and a refresh token (7-day expiry).<br>7. System stores the refresh token in an httpOnly, Secure, SameSite=Strict cookie.<br>8. System returns the access token and user profile to the frontend.<br>9. Frontend stores the access token in memory and redirects to /dashboard. |
| **Postconditions** | The student is authenticated. Access and refresh tokens are issued. |
| **Alternative Flows** | AF1 — Invalid credentials: System returns HTTP 401; frontend displays 'Invalid email or password.' (No indication of which field is wrong, to prevent account enumeration.)<br>AF2 — Account not found: Same HTTP 401 response and message as AF1. |
| **Functional Requirements** | FR-AUTH-06: Authenticate students using email and password.<br>FR-AUTH-07: Issue short-lived access tokens (15-minute expiry) and long-lived refresh tokens (7-day expiry) on successful login.<br>FR-AUTH-08: Store refresh tokens in httpOnly, Secure, SameSite=Strict cookies.<br>FR-AUTH-09: Not differentiate between 'email not found' and 'wrong password' in the error message returned to the client. |
...
Tool: Draw.io (or equivalent UML-compliant tool)  
Contents: Show the **Authenticated Student** actor initiating access to protected platform pages inside the system boundary. Include representative authenticated destinations such as Student Dashboard, Learning Unit list, Lesson Overview, Scenario Practice Workspace, and Review Mode. No internal token-renewal process should be modeled as a separate use case oval.
[PLACEHOLDER — Insert diagram here]
...
**Corresponds to:** General Objective 1 — Establish the foundational Git mental model students need before entering scenario-driven practice, so that 100% of students who proceed to scenario practice have completed all Unit 1 orientation lessons — ensuring no student encounters the Scenario Practice Workspace without first engaging with the platform's conceptual groundwork and interface conventions.

**Module Overview:** This module covers Unit 1 orientation lesson access, interactive concept walkthroughs, and orientation completion gating. Unit 1 contains structured, no-terminal Git orientation content that students complete before entering scenario-driven practice. Each orientation lesson is a consequence-safe, interactive concept walkthrough — not a scored quiz — that uses animated DAGs and click-through elements to build the student's foundational mental model of Git. Orientation completion is enforced by an Orientation Completion Gate: a student who has not completed all Unit 1 lessons is prompted to do so before starting a scenario session for the first time. Orientation Completion Gate adherence is the primary KPI for this module. The Unit 1 orientation content is authored and seeded before deployment and is not administrator-configurable in the current release.
...
- SO 1.2 — 100% of students who start at least one scenario session have completed all Unit 1 orientation lessons beforehand, verified through session log timestamps confirming that Unit 1 lesson completion precedes the first scenario session start (OCG = 100%)
...
| **Main Flow** | 1. Student navigates to the Unit 1 orientation area from the Student Dashboard or from an orientation prompt.<br>2. System displays the list of Unit 1 orientation lessons.<br>3. Student selects a lesson.<br>4. System renders the orientation lesson screen: a no-terminal, concept-only screen featuring an animated DAG or diagram illustrating the Git concept, explanatory text, and an interactive click-through element the student advances by clicking through each stage.<br>5. Student reads the content and navigates between orientation lessons.<br>6. Student may proceed to complete each lesson or browse lessons in any order. |
| **Postconditions** | The student can view all Unit 1 orientation lessons and their current completion status. |
| **Alternative Flows** | AF1 — No Unit 1 lessons are seeded: System displays a message indicating that orientation content is not yet available.<br>AF2 — Student navigates away mid-lesson: Progress within the current lesson is not persisted. The lesson restarts from the beginning on the next visit. |
| **Functional Requirements** | FR-ORI-01: Display the Unit 1 orientation lesson list to all authenticated students.<br>FR-ORI-02: Each orientation lesson shall be a no-terminal, concept-only walkthrough.<br>FR-ORI-03: Each orientation lesson shall include an animated or static DAG illustration and an interactive click-through element. No quiz question or correct-answer requirement shall be used.<br>FR-ORI-04: The orientation lesson list shall display the completion status for each lesson. |
...
Tool: Draw.io, Figma, or equivalent
Contents: Show the orientation lesson list with completion indicators, a selected lesson, and navigation between lessons.
[PLACEHOLDER — Insert diagram here]
...
| **Main Flow** | 1. Student selects a Unit 1 lesson (for example: 1-A — The Three File Areas: working tree, staging area, repository; 1-B — Tracked vs. Untracked Files; 1-C — What HEAD Is).<br>2. System renders the orientation lesson screen: a no-terminal, concept-only screen featuring an animated DAG or diagram illustrating the concept, explanatory text, and an interactive click-through element.<br>3. Student reads the content and clicks through all interactive animation steps.<br>4. When the student has advanced through all required interactive steps, the system marks the lesson as complete in the student's progress record and updates the Unit 1 completion status on the dashboard. No answer correctness check is performed.<br>5. Student may proceed to the next orientation lesson or navigate to a practice unit at any time. |
| **Postconditions** | The selected orientation lesson is marked as complete in the student's progress record once all interactive animation steps have been advanced through. |
| **Alternative Flows** | AF1 — Student exits mid-lesson: Progress within the current lesson is not persisted. The lesson restarts from the beginning on the next visit. |
| **Functional Requirements** | FR-ORI-05: Provide eight interactive, no-terminal orientation lessons in Unit 1, covering foundational Git concepts, command structure and anatomy, DAG literacy, platform conventions, and other orientation content approved for the starter syllabus.<br>FR-ORI-06: Mark an orientation lesson as complete when the student has advanced through all interactive animation steps, without evaluating answer correctness.<br>FR-ORI-07: Persist orientation lesson completion status in the student's progress record.<br>FR-ORI-08: The orientation lesson interface shall not render a terminal input, terminal command area, or scenario evaluation mechanism. |
...
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the **Authenticated Student** actor as the initiating subject. The use case oval "Complete a Unit 1 Orientation Topic" is inside the system boundary. Show that completion is triggered by the student advancing through all interactive animation steps — not by a correctness check.
[PLACEHOLDER — Insert diagram here]

*Diagram 2 — Activity Diagram*
Title: Activity Diagram — Complete a Unit 1 Orientation Topic
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the student selecting a lesson, advancing through animation steps, system marking completion, progress record update, and the alternative flow for mid-lesson exit.
[PLACEHOLDER — Insert diagram here]

*Diagram 3 — Wireframe*
Title: Wireframe — Unit 1 Orientation Lesson Screen (mid-lesson and completed state)
Tool: Draw.io, Figma, or equivalent
Contents: Show the concept walkthrough screen with the animated DAG or diagram, click-through step indicator, and post-completion state showing the lesson as complete.
[PLACEHOLDER — Insert diagram here]

---

### 1.3 Supporting Functional Requirement Group: Orientation Completion Gate

| Field | Description |
|-------|-------------|
| **Requirement Group ID** | FRG-ORI-01 |
| **Requirement Group Name** | Orientation Completion Gate |
| **Classification** | Supporting functional requirement group, not a student-initiated use case |
| **External Actor** | None. The gate is enforced automatically before a student's first scenario session starts. |
| **Triggering Events** | Student attempts to start the first scenario session;<br>system checks Unit 1 orientation completion status before session initialization. |
| **System Responsibilities** | 1. Check whether all Unit 1 orientation lessons are complete before the student's first scenario session is initialized.<br>2. Allow the session to proceed when all orientation lessons are complete.<br>3. Display an orientation completion prompt when one or more lessons are incomplete.<br>4. Log OCG compliance in the session-start record. |
| **Postconditions** | Every first scenario session start is checked against Unit 1 orientation completion status. OCG is computable from session-start logs. |
| **Exception Handling** | EX1 — Progress record unavailable at session start: System rejects session initialization, logs the error, and prompts the student to refresh.<br>EX2 — Student completes remaining lessons and returns: Gate check passes on the next session-start attempt. |
| **Functional Requirements** | FR-ORI-09: Before a student's first scenario session is initialized, the system shall verify that all Unit 1 orientation lessons are marked complete in the student's progress record.<br>FR-ORI-10: If any Unit 1 lesson is not complete, the system shall display an orientation completion prompt before the scenario session starts and prevent session initialization until all lessons are complete.<br>FR-ORI-11: The system shall log OCG compliance for each first-session start as a boolean field in the session record.<br>FR-ORI-12: The Orientation Completion Gate shall apply only to the student's first-ever scenario session; subsequent sessions shall not be re-gated. |
| **Diagram Guidance** | Do not create a standalone use case diagram for the Orientation Completion Gate. If documentation requires a diagram, represent the gate check as part of the scenario session initialization activity or sequence flow. |
---
...
**Module Overview:** This module covers the core student scenario loop: viewing a unit Lesson Overview, opening the unit's scenario list, selecting a scenario and difficulty level, loading a scenario instance, entering Git commands, simulating repository-state changes, evaluating outcomes by final repository state, completing a scenario, and re-attempting a completed scenario difficulty through Review Mode. Scenario selection may begin from two student-facing locations: (1) the Lesson Overview page, where the student clicks **View Scenarios**, and (2) the Dashboard/Units tab, where the student expands a unit card to display the scenarios under that unit. Both paths lead to the same scenario selection and difficulty-selection behavior. Each scenario difficulty instance applies an authored command-count policy. Success is measured primarily by Scenario Completion Rate, Command Accuracy Rate, Average Retry Count, and Review Mode SCR. The State-Based Evaluator accepts any valid command sequence that produces the correct final repository state and does not rely on brittle command-string matching.
...
|---|---|---|
| Learning Unit | Groups related Lesson Overview content and scenarios for students | Seeded content record in the current release; administrator-managed only through the planned Phase 2 Administrative Management module |
| Lesson Overview | Provides scrollable conceptual context for the selected learning unit | Opened from the Dashboard/Units tab or Learning Unit list; includes a **View Scenarios** action |
| Scenario List / View Scenarios Panel | Displays the scenario skill focuses belonging to the selected learning unit | Opened either from the Lesson Overview page or by expanding a unit card in the Dashboard/Units tab |
| Scenario Skill Focus | Represents a reusable Git problem pattern practiced by students | Assigned to a seeded learning unit; exact titles and counts may be finalized before deployment |
| Difficulty Instance | Provides Easy, Medium, and Hard practice levels for a scenario skill focus | Scaffolding rules are fixed; content details are pre-authored before deployment |
...
| Retry Variant | Provides structurally changed attempts for transfer measurement | Required only for RTA-eligible scenarios; exact variant count is finalized before deployment |
...
### 2.1 View Unit Lesson Overview and Open View Scenarios

| Field | Description |
|-------|-------------|
| **Use Case ID** | UC-DASH-02 |
| **Use Case Name** | View Unit Lesson Overview and Open View Scenarios |
| **Actor(s)** | Authenticated Student |
...
| **Main Flow** | 1. Student selects a learning unit by opening the unit card or clicking **View Lesson Overview**.<br>2. System loads and renders the unit-level Lesson Overview page for that learning unit.<br>3. The Lesson Overview displays the saved or pre-authored HTML content and scoped CSS for the selected unit as a single scrollable instructional page. The page may include concept explanations, examples, callouts, command snippets, diagrams, or DAG illustrations as determined by the authored content.<br>4. Student reads or scrolls through the overview.<br>5. Student clicks **View Scenarios**.<br>6. System displays the View Scenarios panel for the same learning unit, listing the scenario skill focuses under that unit with each scenario's short description, completion status, retry indicators, and difficulty access states.<br>7. Student may select a scenario and proceed to difficulty selection under UC-DASH-04. |
| **Postconditions** | Student has been given conceptual context for the unit and can view the scenarios belonging to that unit before any command input is required. |
| **Alternative Flows** | AF1 — Student opens scenarios without reading the full Lesson Overview: System still allows scenario selection because Lesson Overview viewing is recommended context, not a global gate.<br>AF2 — Unit has no published scenarios: System displays an empty-state message and prevents difficulty selection for that unit. |
| **Functional Requirements** | FR-DASH-10: Display a unit-level Lesson Overview page before scenario practice for each published learning unit.<br>FR-DASH-11: The Lesson Overview shall render the saved or pre-authored lesson HTML and scoped CSS for the selected learning unit as a single scrollable instructional page.<br>FR-DASH-12: The Lesson Overview action that opens the scenario list shall be labeled **View Scenarios**.<br>FR-DASH-13: The View Scenarios panel shall list only the scenario skill focuses assigned to the selected learning unit.<br>FR-DASH-14: The View Scenarios panel shall show each listed scenario's Easy, Medium, and Hard access states before the student loads a scenario instance. |

**UC-DASH-02 — Required Diagrams**

*Diagram 1 — Use Case Diagram*
Title: Use Case Diagram — View Unit Lesson Overview and Open View Scenarios
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the **Authenticated Student** actor as the initiating subject. Include use cases "View Unit Lesson Overview" and "Open View Scenarios" inside the system boundary. Show the **View Scenarios** action as the trigger that leads to the scenario list. The student is the sole actor — no system actor appears as a primary initiator.
[PLACEHOLDER — Insert diagram here]

*Diagram 2 — Activity Diagram*
Title: Activity Diagram — View Unit Lesson Overview and Open View Scenarios
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the student opening the unit, reading the Lesson Overview, clicking **View Scenarios**, and seeing the unit's scenario list. Include an alternative flow for a unit with no published scenarios.
[PLACEHOLDER — Insert diagram here]

*Diagram 3 — Wireframe*
Title: Wireframe — Lesson Overview Page with View Scenarios Action
Tool: Draw.io, Figma, or equivalent
Contents: Show the Lesson Overview page as a scrollable instructional page with a clear **View Scenarios** button leading to the scenario list for the same unit.
[PLACEHOLDER — Insert diagram here]

---

### 2.2 Browse Unit Scenarios and Select Scenario Difficulty Level

| Field | Description |
|-------|-------------|
| **Use Case ID** | UC-DASH-04 |
| **Use Case Name** | Browse Unit Scenarios and Select Scenario Difficulty Level |
| **Actor(s)** | Authenticated Student |
| **Preconditions** | Student is authenticated and is viewing a unit's scenario list from either: (a) the Lesson Overview page after clicking **View Scenarios**, or (b) the Dashboard/Units tab after expanding a unit card. Easy is available by default for every scenario. Medium and Hard may be locked depending on the student's completion status for the same scenario. |
| **Main Flow** | 1. Student opens a unit's scenario list from one of the two supported entry points.<br>2. System displays scenario cards under the selected unit. Each scenario card shows scenario title, short narrative/skill focus, completion status, retry indicators, and Easy/Medium/Hard difficulty states.<br>3. Student selects a scenario.<br>4. System displays the Easy, Medium, and Hard difficulty indicators for that scenario with their current access state: Available, Locked, In Progress, or Complete.<br>5. Student selects an available difficulty level.<br>6. System triggers UC-SCENE-01 to load the corresponding scenario instance.<br>7. Difficulty progression follows the scenario-specific mastery gate: Easy is initially available, Medium unlocks only after Easy completion for the same scenario, and Hard unlocks only after Medium completion for the same scenario. |
| **Postconditions** | The selected available scenario instance loads at the chosen difficulty level, or the system displays the requirement needed to unlock the selected locked level. |
| **Alternative Flows** | AF1 — Locked difficulty selected: System does not load the scenario instance and displays the required prerequisite.<br>AF2 — Completed difficulty selected: System allows replay as a new session while preserving the existing completion record and prior logs.<br>AF3 — Student expands a different unit card from the Dashboard/Units tab: System collapses or de-emphasizes the previous unit's scenario list and displays the selected unit's scenarios. |
| **Functional Requirements** | FR-DASH-15: The Dashboard/Units tab shall allow a student to expand a learning unit card to display the scenarios belonging to that unit without requiring the student to first open the Lesson Overview page.<br>FR-DASH-16: The View Scenarios panel opened from the Lesson Overview page and the expanded scenario list opened from the Dashboard/Units tab shall use the same scenario records, difficulty access rules, and completion data.<br>FR-DASH-17: Display all three difficulty levels (Easy, Medium, Hard) for every scenario with their current access/status state: Available, Locked, In Progress, or Complete.<br>FR-DASH-18: Gate Medium behind completion of Easy for the same scenario and gate Hard behind completion of Medium for the same scenario.<br>FR-DASH-19: Display the completion status (Not Started / In Progress / Complete) for each difficulty level per scenario.<br>FR-DASH-20: Scenario selection shall not be globally gated by unit completion, quiz scores, manual approval, or completion of unrelated scenarios. |

**UC-DASH-04 — Required Diagrams**

*Diagram 1 — Use Case Diagram*
Title: Use Case Diagram — Browse Unit Scenarios and Select Scenario Difficulty Level
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the **Authenticated Student** actor as the initiating subject. Include use cases "Browse Unit Scenarios," "Expand Unit Card," and "Select Difficulty Level" inside the system boundary. Show that both entry points (Lesson Overview and Dashboard/Units tab) lead to the same "Select Difficulty Level" use case. No system actor appears as a primary initiator.
[PLACEHOLDER — Insert diagram here]

*Diagram 2 — Activity Diagram*
Title: Activity Diagram — Browse Unit Scenarios and Select Scenario Difficulty Level
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show both entry paths: Lesson Overview → **View Scenarios** and Dashboard/Units tab → expand unit card. Both should merge into the same scenario selection, difficulty access check, and scenario-instance load decision.
[PLACEHOLDER — Insert diagram here]

*Diagram 3 — Wireframe*
Title: Wireframe — Unit Scenario List with Difficulty Selection
Tool: Draw.io, Figma, or equivalent
Contents: Show a unit card expanded with scenarios listed below it and difficulty selectors on each scenario card. Also show the equivalent View Scenarios panel from the Lesson Overview page.
[PLACEHOLDER — Insert diagram here]
...
| **Actor(s)** | Authenticated Student |
| **Preconditions** | A scenario and an available difficulty level have been selected from either entry path. The Orientation Completion Gate check (FRG-ORI-01) has been satisfied. Difficulty access is controlled only by the scenario-specific Easy → Medium → Hard completion progression. |
| **Main Flow** | 1. Student selects a scenario and an available difficulty level from one of the two supported entry paths.<br>2. System validates that the selected scenario belongs to the selected learning unit and that the selected difficulty is unlocked.<br>3. System loads the scenario definition, difficulty instance, authored command-count policy, and selected variant from the database, or selects the next structurally valid retry variant when the student's prior attempt requires adaptive retry behavior.<br>4. System initializes a new scenario session: creates an active session record, initializes the Repository State Simulator with the scenario's starting repository state, records the source entry point, stores the applicable command-count policy for the session, and logs session start time and variant ID through FRG-LOG-01.<br>5. System renders the scenario interface with: (a) the narrative context card, (b) the terminal-style command input, auto-focused and keyboard-ready, (c) the live animated DAG showing the starting repository state, (d) the expected-state diagram on Easy and Medium only, (e) the contextual feedback panel area on Easy only, visible but blank until a command is processed by the simulator, and (f) the Remaining Counted-Command Counter.<br>6. Student is ready to enter Git commands. |
...
| **Functional Requirements** | FR-SCENE-01: Load a valid variant from the scenario's variant pool on each new session start after validating that the selected difficulty is unlocked.<br>FR-SCENE-02: Initialize the Repository State Simulator with the selected variant's starting repository state.<br>FR-SCENE-03: Display the appropriate interface components based on difficulty level: Easy (live DAG + expected-state diagram + contextual feedback panel); Medium (live DAG + expected-state diagram); Hard (live DAG + narrative context only).<br>FR-SCENE-04: Create and persist a session log record on scenario load through FRG-LOG-01, including selected scenario, difficulty level, variant ID, timestamp, source entry point, and applicable command-count policy ID or snapshot.<br>FR-SCENE-05: The terminal input shall be the default focused element when a scenario loads so students can begin typing immediately.<br>FR-SCENE-05A: The system shall support loading the same scenario instance behavior from both the Lesson Overview → View Scenarios path and the Dashboard/Units tab → expanded unit card path.<br>FR-SCENE-05B: Each loaded scenario difficulty instance shall include an authored command-count policy containing a minimum counted-command threshold, maximum counted-command limit, and non-counted diagnostic command patterns. |
...
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the **Authenticated Student** actor as the initiating subject. Show "Load a Scenario Instance" inside the system boundary. Show the two preceding use cases (from Lesson Overview path and Dashboard/Units tab path) as separate entry-point use cases that both include or precede "Load a Scenario Instance." Do not model logging as a student use case.
[PLACEHOLDER — Insert diagram here]
...
| **Postconditions** | Completion record is updated. Stars and streaks are awarded if applicable. The Review button for this scenario instance becomes visible on the dashboard. |
...
| **Main Flow** | 1. Student clicks the Review button for a completed scenario difficulty instance.<br>2. System verifies that the student has a completed session for the selected scenario difficulty.<br>3. System initializes a new Review Mode session for that scenario difficulty and records the session as review_mode = true.<br>4. System loads the scenario narrative, starting repository state, and selected variant according to the platform's replay/re-attempt rules.<br>5. System renders the Scenario Practice Workspace with the live terminal, live DAG, and the same difficulty-appropriate scaffolding used in regular practice: Easy shows live DAG, expected-state diagram, and contextual feedback; Medium shows live DAG and expected-state diagram; Hard shows live DAG and narrative context only.<br>6. Student enters Git commands through the terminal.<br>7. System processes commands through the Repository State Simulator and State-Based Evaluator using the same state-based evaluation rules in UC-SCENE-02.<br>8. If the target state is reached, system marks the Review Mode session as completed and includes it in Review Mode SCR.<br>9. Student exits to the dashboard or chooses another available scenario action. |
...
**Module Overview:** This module defines the visual and instructional supports available during scenario practice. The live animated DAG remains visible at all difficulty levels. The expected-state diagram appears only on Easy and Medium levels. The contextual feedback panel appears only on Easy level and describes the consequence of the student's own processed command without revealing the correct command, command sequence, or target repository state. These are student-visible learning supports, documented as student-observable use cases.
...
| **Main Flow** | 1. Student loads a scenario instance.<br>2. System renders the starting repository state in the Live Animated DAG panel.<br>3. Student enters a Git command that maps to the simulator's implemented operation set.<br>4. System processes the command through the Repository State Simulator and returns the updated repository state.<br>5. System updates the DAG to show current commit nodes, branch pointer labels, HEAD position, parent-child commit edges, staging/working-tree markers when applicable, and conflict-state markers when applicable.<br>6. Student observes how the command changed the repository state.<br>7. This process repeats after each simulator-processed command. |
...
| **Functional Requirements** | FR-DAG-01: The live DAG shall be present and visible at all difficulty levels (Easy, Medium, and Hard).<br>FR-DAG-02: The live DAG shall update after every simulator-processed command to reflect the current internal repository state.<br>FR-DAG-03: The DAG shall render commit nodes with short SHA identifiers and commit messages, branch pointer labels at their current commit, HEAD pointer position (attached or detached), parent-child commit edges, and required state markers when applicable.<br>FR-DAG-04: State transitions shall be animated, with animation timing governed by NFR-PERF-02.<br>FR-DAG-05: The DAG panel shall remain visible during a scenario session without requiring horizontal scrolling on a 1280×720 display. |
...
| **Actor(s)** | Authenticated Student |
| **Preconditions** | Student has a prior scenario session for the same scenario skill focus that ended with status Failed or Abandoned, then starts another attempt. A variant pool exists for that scenario skill focus. |
| **Main Flow** | 1. Student chooses to retry a scenario skill focus after a prior session ended as Failed or Abandoned.<br>2. System checks the most recent variant ID used by the student for that scenario skill focus and whether the prior session outcome was Failed or Abandoned.<br>3. System selects a structurally different variant from the available variant pool when one exists.<br>4. Variant differences may include branch names, commit topology, file state, conflict setup, or starting repository state.<br>5. System initializes the scenario using the selected variant and records the variant ID and prior-session relationship through FRG-LOG-01.<br>6. Student receives a retry attempt that practices the same Git concept in a changed repository structure. |
...
| **System Responsibilities** | 1. Create a session record on scenario start.<br>2. Write a step log entry after each command execution.<br>3. Update the session record on completion, failure, abandonment, or retry.<br>4. Store variant IDs and prior-session references where applicable.<br>5. Tag Review Mode sessions separately from primary scenario attempts.<br>6. Preserve all data needed for OCG, SCR, ARC, CAR, HLCR, RTA, SAR, Review Mode SCR, dashboard progress, first-attempt stars, and streaks.<br>7. Preserve command-count policy snapshots, command classifications, counted action-command totals, and non-counted diagnostic command totals required for CAR and remaining counted-command calculations. |
...
| **Functional Requirements** | FR-LOG-01: The system shall create a session record on scenario load capturing student ID, learning unit ID, scenario ID, difficulty level, variant ID, source entry point, session mode, prior-session reference when applicable, and start timestamp.<br>FR-LOG-02: The system shall write a step log entry after every command submission, including TargetMatched, TargetNotYetMatched, Unprocessable, and Invalid results. TargetNotYetMatched is a step-level result and shall not by itself set the whole scenario session to Failed.<br>FR-LOG-03: Step log entries shall record the command entered, evaluation result category, command classification, counted-command increment value, attempt number for the current step, resulting state hash when applicable, expected target state hash when applicable, and timestamp.<br>FR-LOG-04: The system shall persist total attempts, total counted action commands, total non-counted diagnostic commands, total retries, completion status, failure status, abandonment status, variant ID, command-count policy ID or snapshot, and first-attempt-star eligibility on session completion, failure, or abandonment.<br>FR-LOG-05: The system shall flag a session as RTA-eligible only when: (a) the session is not the student's first-ever session for that scenario skill focus, (b) the student has a prior session with status Failed or Abandoned for the same scenario skill focus, and (c) the served variant is structurally different from the relevant prior Failed or Abandoned session.<br>FR-LOG-06: The RTA success flag shall be set to true only when the RTA-eligible changed-variant retry session is completed without any non-target-matching, unprocessable, or invalid command submission.<br>FR-LOG-07: The system shall log OCG compliance for each first-session start as a boolean field confirming whether all Unit 1 orientation lessons were complete at the time of first session initialization.<br>FR-LOG-08: All log writes shall be atomic to prevent partial records.<br>FR-LOG-09: Abandoned session data shall be retained and clearly marked as Abandoned.<br>FR-LOG-10: Review Mode sessions shall be tagged separately from primary scenario attempts so Review Mode SCR can be computed independently from primary SCR, CAR, RTA, and progression records. |
...
| **Main Flow** | 1. Student navigates to the Student Dashboard.<br>2. System retrieves the student's unit completion records, scenario completion records, streak data, first-attempt star flags, retry-count trends, Hard-level completion indicators, RTA progress, and session log aggregates from the database.<br>3. System renders dashboard summary indicators and one card for each published learning unit.<br>4. Each unit card displays unit title, short description, progress bar, lesson overview action, and an expand/collapse control.<br>5. Student expands a unit card.<br>6. System displays the scenarios belonging to that unit directly below the expanded card.<br>7. Each scenario row/card displays scenario title, skill focus, completion status for Easy/Medium/Hard, difficulty access states, retry indicators, and Review buttons where available.<br>8. Student may select an available difficulty level directly from the expanded unit card, which triggers UC-SCENE-01.<br>9. System displays the active streak counter, first-attempt star indicators, retry-count trends, KPI summary indicators, and improvement indicators derived exclusively from logs. |
| **Postconditions** | Student sees current learning status, progress bars, KPI summary indicators, improvement indicators, expanded unit scenario lists, difficulty access states, and review access across all published learning units. |
| **Alternative Flows** | AF1 — Newly registered student with no progress data: All scenario instances show as Not Started, streak counter is 0, KPI indicators display No data available, progress bars display 0%, and no Review buttons are visible.<br>AF2 — Student collapses unit card: System hides the scenario list while preserving dashboard summary data.<br>AF3 — Unit has no published scenarios: System displays an empty-state message under the expanded unit card. |
| **Functional Requirements** | FR-DASH-01: Display a unit card for each published learning unit.<br>FR-DASH-02: Display per-scenario completion status at each difficulty level (Easy/Medium/Hard).<br>FR-DASH-03: Display the active streak counter updated after each completed scenario session.<br>FR-DASH-04: Display first-attempt star indicators for qualifying scenario completions with no non-target-matching, unprocessable, or invalid command submissions during the entire session.<br>FR-DASH-05: Display retry-count trends per scenario derived from completed and abandoned session logs; when insufficient historical data exists, display No trend available.<br>FR-DASH-06: Each unit card shall include a progress bar displaying the percentage of scenario instances completed out of the total instances in that unit across all three difficulty levels.<br>FR-DASH-07: Display KPI summary indicators for the active published scenario library and, when data is available, for each active learning unit using OCG, SCR, ARC, CAR, HLCR, RTA, and SAR values derived exclusively from session logs; when a KPI denominator is zero, display No data available rather than 0%. During acceptance testing, displayed KPI values shall match the corresponding source logs for the approved test cases.<br>FR-DASH-08: Display a Review button on each scenario instance with at least one completed session record.<br>FR-DASH-09: For any active learning unit or scenario skill focus with incomplete scenarios, high retry counts, low Hard-level completion, or unmet transfer targets, render a visual improvement indicator adjacent to the KPI summary indicator.<br>FR-DASH-09A: The Dashboard/Units tab shall support expanding a unit card to display that unit's scenario list below the card.<br>FR-DASH-09B: The expanded unit-card scenario list shall allow the student to select an available difficulty level without first opening the Lesson Overview page. |
...
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show dashboard load, unit cards rendering, unit expansion, scenario list display, difficulty selection, locked-state handling, and transition to scenario instance loading.
[PLACEHOLDER — Insert diagram here]

*Diagram 3 — Wireframe*
Title: Wireframe — Dashboard/Units Tab with Expanded Unit Card
Tool: Draw.io, Figma, or equivalent
Contents: Show unit cards, one expanded unit card with scenario rows/cards underneath, difficulty selectors, completion indicators, retry indicators, and Review buttons.
[PLACEHOLDER — Insert diagram here]
...
| **Actor(s)** | Authenticated Student |
| **Preconditions** | The student is viewing the dashboard, an expanded unit scenario list, or a scenario completion screen. At least one completed session exists for the selected scenario difficulty instance. |
| **Main Flow** | 1. Student views a scenario difficulty instance that has already been completed.<br>2. System displays a Review button for that completed scenario difficulty instance.<br>3. Student clicks the Review button.<br>4. System validates that the selected scenario difficulty has at least one completed session for the student.<br>5. System routes the student to UC-REVIEW-01 under Module 2 to start a playable Review Mode session. |
| **Postconditions** | The student is routed to Review Mode for the selected completed scenario difficulty instance. |
...
| **Functional Requirements** | FR-DASH-23: Display a Review button only for scenario difficulty instances completed by the student.<br>FR-DASH-24: Allow Review Mode access from the dashboard/unit scenario card and from the completion screen.<br>FR-DASH-25: Validate completion ownership before routing the student to Review Mode.<br>FR-DASH-26: Route valid Review actions to the playable Review Mode session behavior specified in UC-REVIEW-01 under Module 2.<br>FR-DASH-27: Preserve dashboard progress records when the student starts Review Mode; Review Mode attempts shall be logged separately through FRG-LOG-01. |
...
### 6.4 Manage Learning Units and Lesson Overview Content

| Field | Description |
|-------|-------------|
| **Use Case ID** | UC-ADMIN-04 |
| **Use Case Name** | Manage Learning Units and Lesson Overview Content |
| **Actor(s)** | Administrator (Authenticated) |
...
| **Main Flow** | 1. Administrator opens the Learning Unit Management screen.<br>2. System displays the existing learning content units in the active content library.<br>3. Administrator chooses to add a new learning unit or edit an existing learning unit.<br>4. System displays editable unit fields, including unit title, unit description, ordering, publication status, and content-source label.<br>5. Administrator opens the Lesson Overview content area for the selected learning unit.<br>6. System displays an AI-assisted Lesson Overview authoring interface containing: (a) an AI Lesson Assistant chat panel, (b) a Code Editor with an internal HTML/CSS toggle, and (c) a Preview panel.<br>7. Administrator may manually edit the Lesson Overview HTML and scoped CSS through the Code Editor, or may request AI assistance through UC-AI-02 to generate or revise the HTML/CSS content.<br>8. If AI assistance is used, the system displays the AI explanation in the chat panel and presents proposed HTML/CSS changes for administrator review; AI output is not saved or published automatically.<br>9. Administrator previews the rendered Lesson Overview as it will appear to students.<br>10. Administrator saves the Lesson Overview as draft or publishes it.<br>11. System sanitizes the HTML, scopes CSS to the Lesson Overview container, validates that the content does not expose scenario answers, saves the approved change, and records the administrative action in the audit log. |
| **Postconditions** | A learning unit and its Lesson Overview HTML/CSS content are created, updated, archived, restored, saved as draft, or published, and the administrative action is logged. |
| **Alternative Flows** | AF1 — Administrator attempts to delete a learning unit with existing scenario/session records: System prevents deletion and offers archive or draft/unpublish actions instead.<br>AF2 — Content violates the No-Answer Policy: System rejects publication and displays a validation message.<br>AF3 — Administrator cancels editing: System discards unsaved changes after confirmation.<br>AF4 — Administrator creates a new learning unit: System labels the learning unit as a content extension and excludes it from original KPI targets unless the evaluation framework is formally revised.<br>AF5 — Administrator imports AI-generated Lesson Overview content: System presents proposed HTML and scoped CSS changes in the editor/preview workflow and still requires administrator review and publication.<br>AF6 — HTML or CSS contains unsafe content: System blocks scripts, unsafe attributes, unsafe external embeds, or CSS rules that can affect application areas outside the Lesson Overview container. |
| **Functional Requirements** | FR-ADMIN-15: Allow the administrator to create, edit, archive, restore, save as draft, and publish learning content units.<br>FR-ADMIN-16: Allow the administrator to maintain Lesson Overview content for each learning unit as HTML and scoped CSS.<br>FR-ADMIN-17: Label learning units by content source or status.<br>FR-ADMIN-18: Validate that published unit and Lesson Overview content does not expose correct scenario commands or command sequences.<br>FR-ADMIN-19: Record every learning-unit and Lesson Overview content change in the administrative audit log.<br>FR-ADMIN-20: Provide a Lesson Overview Code Editor with an internal HTML/CSS toggle and a rendered Preview panel.<br>FR-ADMIN-21: Sanitize Lesson Overview HTML before saving or rendering and block scripts, unsafe attributes, and unsafe embeds.<br>FR-ADMIN-22: Scope Lesson Overview CSS to the Lesson Overview container so that administrator-authored CSS cannot affect unrelated application pages or layout components.<br>FR-ADMIN-23: Allow AI-generated Lesson Overview HTML/CSS drafts from UC-AI-02 to be reviewed, edited, previewed, accepted, discarded, saved, or published by the administrator, but prevent direct AI publication without administrator approval. |

**UC-ADMIN-04 — Required Diagrams**

*Diagram 1 — Use Case Diagram*
Title: Use Case Diagram — Manage Learning Units and Lesson Overview Content
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the **Administrator** actor; include unit creation/editing, Lesson Overview HTML/CSS editing, previewing, saving/publishing, and an <<extend>> relationship from Generate Lesson Overview HTML/CSS Draft Using AI Assistance (UC-AI-02) when AI assistance is enabled.
[PLACEHOLDER — Insert diagram here]

*Diagram 2 — Activity Diagram*
Title: Activity Diagram — Manage Learning Units and Lesson Overview Content
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the administrator selecting or creating a learning unit, opening the Lesson Overview HTML/CSS editor, optionally requesting AI assistance, reviewing proposed HTML/CSS changes, previewing rendered output, validating No-Answer Policy and HTML/CSS safety rules, saving/publishing, and logging the action.
[PLACEHOLDER — Insert diagram here]

*Diagram 3 — Wireframe*
Title: Wireframe — Administrative Learning Unit and Lesson Overview Management Screen
Tool: Draw.io, Figma, or equivalent
...
| **Main Flow** | 1. Administrator opens the Scenario Management screen.<br>2. System displays existing learning units and their scenario lists.<br>3. Administrator selects a learning unit and chooses to add a new scenario or edit an existing scenario.<br>4. System displays the Scenario Builder form with editable fields: scenario title, scenario skill focus, Git concept tag, narrative context, student task prompt, simulator operation scope, initial repository state setup, target repository state rules, difficulty-level scaffolding settings, command-count policy values, contextual feedback drafts, and scenario variants.<br>5. Administrator defines or edits the initial repository state using a structured state builder or setup-command sequence.<br>6. Administrator defines or edits the target repository state using a structured target-state builder or an administrator-only reference solution sequence; the system stores the resulting target state for evaluation rather than command-string matching.<br>7. System renders the Live DAG preview from the initial repository state and renders the Expected-State Diagram preview from the target repository state.<br>8. Administrator defines or edits Easy, Medium, and Hard difficulty instances, assigns the correct scaffolding visibility for each level, and configures each difficulty instance's minimum counted-command threshold, maximum counted-command limit, and non-counted diagnostic command patterns.<br>9. Administrator defines structurally distinct scenario variants, or imports an AI-generated structured scenario draft from UC-AI-03 into the Scenario Builder form for review.<br>10. Administrator saves the scenario as draft or submits it for publication.<br>11. System validates that required fields are complete, the scenario is assigned to a valid learning unit, all required difficulty instances are present, variant relationships are valid, state definitions are internally consistent, target states are reachable using the simulator's implemented Git operation set, and No-Answer Policy constraints are satisfied.<br>12. System saves or publishes the scenario only if validation passes.<br>13. System records the administrative action in the audit log. |
...
| **Alternative Flows** | AF1 — Required scenario fields are incomplete: System rejects saving or publication and identifies the missing fields.<br>AF2 — Variant pool is incomplete or not structurally distinct: System rejects publication until the administrator supplies valid variants.<br>AF3 — Content exposes a correct command or command sequence on a student-facing surface: System rejects publication.<br>AF4 — No suitable learning unit exists: Administrator creates or edits a learning unit through UC-ADMIN-04, then returns to scenario authoring.<br>AF5 — Administrator imports AI-generated draft content: System loads structured AI draft values into the Scenario Builder form and still requires administrator review and publication.<br>AF6 — Initial or target repository state cannot be simulated, compared, or rendered as a DAG: System rejects publication and identifies the invalid state definition. |
| **Functional Requirements** | FR-ADMIN-24: Allow the administrator to create, edit, archive, restore, and publish scenario records under selected learning units.<br>FR-ADMIN-25: Require each published scenario to belong to exactly one learning unit.<br>FR-ADMIN-26: Require every published scenario skill focus to have Easy, Medium, and Hard difficulty instances, each with a valid command-count policy.<br>FR-ADMIN-26A: Allow the administrator to author the minimum counted-command threshold, maximum counted-command limit, and non-counted diagnostic command patterns for each scenario difficulty instance.<br>FR-ADMIN-27: Require RTA-eligible published scenario skill focuses to maintain structurally distinct variants sufficient to prevent immediate identical retry repetition and support transfer measurement.<br>FR-ADMIN-28: Validate that scenario state definitions are internally consistent, reachable through the simulator's implemented Git operation set, and usable by the State-Based Evaluator before publication.<br>FR-ADMIN-29: Validate that student-facing scenario content does not expose correct commands or correct command sequences.<br>FR-ADMIN-30: Allow AI-generated scenario drafts from UC-AI-03 to populate editable Scenario Builder fields, but prevent direct AI publication without administrator review.<br>FR-ADMIN-31: Generate Live DAG and Expected-State Diagram previews from the same structured repository-state definitions used for simulation and evaluation.<br>FR-ADMIN-32: Record every scenario-management action in the administrative audit log. |
...
| **Actor(s)** | Authenticated Student |
| **Preconditions** | Student is on a Lesson Overview screen (UC-DASH-02). The AI chatbot feature has been enabled. |
| **Main Flow** | 1. Student types a conceptual Git question into the chatbot input on the Lesson Overview screen.<br>2. System sends the question and the current unit's concept context to the AI backend.<br>3. AI backend returns a response addressing the student's conceptual question.<br>4. Chatbot displays the response in the chat panel on the Lesson Overview screen.<br>5. The chatbot operates strictly within conceptual Git education scope; it shall not provide scenario answers, reveal correct commands for any scenario, or violate the No-Answer Policy. |
...
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the **Authenticated Student** actor initiating the query; the chatbot use case is inside the system boundary; show <<include>> from the Lesson Overview context.
[PLACEHOLDER — Insert diagram here]
...
*Diagram 3 — Wireframe*
Title: Wireframe — Lesson Overview with AI Chatbot Panel
Tool: Draw.io, Figma, or equivalent
Contents: Show the Lesson Overview page with the AI chatbot panel, question input, and response area.
[PLACEHOLDER — Insert diagram here]

---

### 7.2 Generate Lesson Overview HTML/CSS Draft Using AI Assistance

| Field | Description |
|-------|-------------|
| **Use Case ID** | UC-AI-02 |
| **Use Case Name** | Generate Lesson Overview HTML/CSS Draft Using AI Assistance |
| **Actor(s)** | Administrator (Authenticated) |
| **Preconditions** | Administrator is authenticated and is in the Lesson Overview authoring workflow under UC-ADMIN-04. The AI Lesson Overview drafting feature has been enabled. |
| **Main Flow** | 1. Administrator opens the AI Lesson Assistant chat panel from the Lesson Overview editor.<br>2. System displays the AI Lesson Assistant alongside the Code Editor and Preview panel.<br>3. Administrator enters a drafting or revision request, such as a target learning unit, conceptual topic, desired content structure, or proposed HTML/CSS changes.<br>4. System sends the request and relevant content context to the AI backend.<br>5. AI backend returns two coordinated outputs: (a) a conversational response in the chat panel explaining the proposed changes and accepting revision requests, and (b) proposed HTML and scoped CSS updates that may be inserted into the Lesson Overview Code Editor after administrator approval.<br>6. System presents the proposed HTML/CSS changes alongside the current Code Editor content for comparison.<br>7. Administrator reviews the proposed changes, may apply, partially apply, edit, or discard them, and may request further revisions from the AI.<br>8. If changes are applied, system inserts them into the Code Editor; the administrator then previews, validates (No-Answer Policy, HTML/CSS safety), and saves or publishes through UC-ADMIN-04. |
| **Postconditions** | A proposed Lesson Overview HTML/CSS draft has been generated and is available in the Code Editor for administrator review. No Lesson Overview content has been published directly by AI. |
...
| **Functional Requirements** | FR-AI-04: Provide an AI-assisted Lesson Overview drafting interface accessible to Administrator users only when the AI feature is enabled.<br>FR-AI-05: Provide an AI assistant chat panel that explains proposed changes and accepts revision requests.<br>FR-AI-06: Generate proposed Lesson Overview content as editable HTML and scoped CSS, not as separate structured lesson-summary fields.<br>FR-AI-07: Allow the AI to propose coordinated HTML and CSS changes in a single generation or revision action.<br>FR-AI-08: Display proposed AI-generated HTML/CSS changes for administrator review before they become the current Lesson Overview draft.<br>FR-AI-09: AI-generated Lesson Overview HTML/CSS drafts shall conform to the No-Answer Policy.<br>FR-AI-10: AI-generated Lesson Overview drafts shall require administrator review, HTML/CSS safety validation, No-Answer Policy validation, and publication through UC-ADMIN-04 before becoming student-accessible.<br>FR-AI-11: The AI feature shall not create, publish, or activate a learning unit or Lesson Overview without administrator approval. |

**UC-AI-02 — Required Diagrams**

*Diagram 1 — Use Case Diagram*
Title: Use Case Diagram — Generate Lesson Overview HTML/CSS Draft Using AI Assistance
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the **Administrator** actor; show UC-AI-02 as an optional <<extend>> of UC-ADMIN-04 Manage Learning Units and Lesson Overview Content.
[PLACEHOLDER — Insert diagram here]

*Diagram 2 — Activity Diagram*
Title: Activity Diagram — Generate Lesson Overview HTML/CSS Draft Using AI Assistance
Tool: Draw.io (or equivalent UML-compliant tool)
...
*Diagram 3 — Wireframe*
Title: Wireframe — AI Lesson Assistant and Lesson Overview HTML/CSS Editor Integration
Tool: Draw.io, Figma, or equivalent
Contents: Show the AI assistant chat panel, Code Editor with HTML/CSS toggle, proposed-change preview, rendered Lesson Overview preview, and validation warnings.
[PLACEHOLDER — Insert diagram here]
...
|---|---|---|---|---|
| General Objective 1: Establish the foundational Git mental model students need before entering scenario-driven practice, so that 100% of students who proceed to scenario practice have completed all orientation lessons | **Module 1: Orientation and Conceptual Readiness** | One-to-one objective-module | Unit 1 orientation lesson list, individual lesson walkthroughs, animated DAG/interactive click-through, completion marking, Orientation Completion Gate enforcement (FRG-ORI-01), OCG session logging | OCG = 100% gate adherence; 100% lesson accessibility; ARC ≤ 2 as indirect readiness indicator |
| General Objective 2: Improve students' ability to resolve realistic Git repository problems through terminal-based scenario execution and state-based repository evaluation | **Module 2: Scenario Practice and State-Based Evaluation** | One-to-one objective-module | Lesson Overview → View Scenarios, dashboard/unit-card scenario selection, difficulty selection, scenario instance loading, command input, authored command-count policy, repository simulation, state-based evaluation, completion handling, Review Mode | SCR ≥ 80%; ARC ≤ 2; CAR ≥ 60%; Review Mode SCR ≥ 60%; evaluator validation test set |
...
| General Objective 5: Improve progress self-monitoring through visible, log-derived dashboard indicators | **Module 5: Progress Tracking and Self-Monitoring** | One-to-one objective-module | Session/step logging (FRG-LOG-01), log-derived dashboard indicators, unit-card progress display, expanded unit scenario lists, Review button visibility, Review Mode routing | 100% log-display match accuracy; SAR ≤ 20%; dashboard acceptance test set |
...
| Planned Phase 2 / next-semester scope only | **Module 7: AI-Assisted Learning Support** | Planned support module; excluded from current-release MVP KPI computation | Conceptual Git chatbot, Lesson Overview HTML/CSS drafting, scenario draft generation | Phase 2 acceptance tests only |
...
| NFR-SEC-10 | The system shall comply with the Philippine Data Privacy Act of 2012 (RA 10173). Student personal data (name, email) shall be encrypted at rest and in transit. |
