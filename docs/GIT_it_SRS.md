...
| Live Animated DAG | The real-time commit graph rendered on the GIT it! interface; uses circular commit nodes with shortened commit hashes inside, minimal branch/reference labels, and a blue active-state accent for HEAD or the current branch; updates after every command execution at all difficulty levels |
...
**Module Overview:** This module defines the visual and instructional supports available during scenario practice. The live animated DAG remains visible at all difficulty levels and presents the repository graph as clean circular commit nodes, shortened commit hashes, parent-child edges, minimal branch/reference labels, and a blue accent for HEAD or the current active branch state. The expected-state diagram appears only on Easy and Medium levels. The contextual feedback panel appears only on Easy level and describes the consequence of the student's own processed command without revealing the correct command, command sequence, or target repository state. These are student-visible learning supports, documented as student-observable use cases.
...
| **Main Flow** | 1. Student loads a scenario instance.<br>2. System renders the starting repository state in the Live Animated DAG panel.<br>3. Student enters a Git command that maps to the simulator's implemented operation set.<br>4. System processes the command through the Repository State Simulator and returns the updated repository state.<br>5. System updates the DAG to show circular commit nodes with shortened commit hashes inside, parent-child commit edges, minimal branch/reference labels at relevant commit positions, and a blue active-state accent for HEAD or the current branch.<br>6. Student observes how the command changed the repository state.<br>7. This process repeats after each simulator-processed command. |
...
| **Functional Requirements** | FR-DAG-01: The live DAG shall be present and visible at all difficulty levels (Easy, Medium, and Hard).<br>FR-DAG-02: The live DAG shall update after every simulator-processed command to reflect the current internal repository state.<br>FR-DAG-03: The DAG shall render commits as circular nodes with shortened commit hashes inside each circle, parent-child commit edges, minimal branch/reference labels at their current commit, and a clean blue accent indicating HEAD or the current active branch state, including detached HEAD when applicable.<br>FR-DAG-04: State transitions shall be responsive and visually consistent with the scenario workspace, with timing governed by NFR-PERF-02.<br>FR-DAG-05: The DAG panel shall remain visible during a scenario session without requiring horizontal scrolling on a 1280×720 display. |
