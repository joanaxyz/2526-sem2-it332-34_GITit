...
The student-facing interface includes the following core components: registration and login screens, Student Dashboard, Dashboard/Units tab, Learning Unit list, expandable unit cards with lesson lists, Unit 1 Orientation interface, Lesson page, embedded lesson scenario list, Scenario Selection interface, Difficulty Selection/Unlocking interface, Scenario Practice Workspace, terminal-style command input, Live DAG panel, Expected-State Diagram panel, Contextual Feedback panel, Review Mode, and student-facing progress/KPI indicators. Scenario selection begins from a scenario-bearing Lesson page, where the student uses the embedded scenario list after reading the relevant concept lesson. Administrative Management, AI-assisted authoring, and administrator-facing content editing interfaces are planned Phase 2 / next-semester modules, not current-release requirements.
...
| Lesson | A single scrollable instructional page within a learning unit. Stored and displayed as administrator-approved or pre-authored HTML with scoped CSS. May include text, examples, callouts, command snippets, diagrams, or DAG illustrations. Scenario-bearing lessons embed the scenario list attached to that lesson. |
| Scenario List | The scenario-list section embedded under a scenario-bearing Lesson. Lists that lesson's scenario skill focuses and their Easy, Medium, and Hard access states. |
| Unit Card Expansion | The Dashboard/Units tab interaction where a student expands a learning unit card to display the lessons belonging to that unit |
...
- Scenario selection is reachable from scenario-bearing Lesson pages through an embedded scenario list. Unit cards expand to show lessons first, so practice stays tied to the concept lesson that prepares the student for the scenario.
...
|-----------|-------------|
| Frontend SPA (React.js / Vite) | Browser-based student interface for registration, login, dashboard, Units view, Lesson pages, scenario selection, Scenario Practice Workspace, terminal input, live DAG visualization, expected-state diagram, contextual feedback panel, Review Mode, and progress indicators. Communicates with the backend exclusively through RESTful HTTPS endpoints using JSON payloads. |
...
| Lesson | Provides scrollable conceptual context for a specific topic within the selected learning unit | Opened from the Dashboard/Units tab or Learning Unit list; scenario-bearing lessons include an embedded scenario list |
| Scenario List | Displays the scenario skill focuses belonging to the selected lesson | Embedded under scenario-bearing Lesson pages |
...
### 2.1 View Lesson and Browse Attached Scenarios

| Field | Description |
|-------|-------------|
| **Use Case ID** | UC-DASH-02 |
| **Use Case Name** | View Lesson and Browse Attached Scenarios |
| **Actor(s)** | Authenticated Student |
...
| **Main Flow** | 1. Student expands a learning unit card and chooses **View lesson** for a specific lesson.<br>2. System loads and renders the selected Lesson page.<br>3. The Lesson displays saved or pre-authored HTML content and scoped CSS as a single scrollable instructional page. The page may include concept explanations, examples, callouts, command snippets, diagrams, or DAG illustrations as determined by the authored content.<br>4. Student reads or scrolls through the lesson.<br>5. If the Lesson is scenario-bearing, the page displays the embedded scenario list for that lesson.<br>6. The scenario list shows the attached scenario skill focuses with each scenario's short description, completion status, retry indicators, and difficulty access states.<br>7. Student may select a scenario difficulty under UC-DASH-04. |
| **Postconditions** | Student has been given concept-specific context and can view any scenarios attached to that lesson before any command input is required. |
| **Alternative Flows** | AF1 — Student opens scenarios without reading the full Lesson: System still allows scenario selection because Lesson viewing is recommended context, not a global gate.<br>AF2 — Lesson has no published scenarios: System displays only the lesson content and no difficulty selection controls. |
| **Functional Requirements** | FR-DASH-10: Display a Lesson page before scenario practice for each scenario-bearing lesson.<br>FR-DASH-11: The Lesson shall render the saved or pre-authored lesson HTML and scoped CSS as a single scrollable instructional page.<br>FR-DASH-12: Scenario-bearing Lesson pages shall embed the attached scenario list directly below the lesson content.<br>FR-DASH-13: The scenario list shall list only the scenario skill focuses assigned to the selected lesson.<br>FR-DASH-14: The scenario list shall show each listed scenario's Easy, Medium, and Hard access states before the student loads a scenario instance. |

**UC-DASH-02 — Required Diagrams**

*Diagram 1 — Use Case Diagram*
Title: Use Case Diagram — View Lesson and Browse Attached Scenarios
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the **Authenticated Student** actor as the initiating subject. Include use cases "View Lesson" and "Browse Attached Scenarios" inside the system boundary. Show the embedded scenario list on scenario-bearing lessons. The student is the sole actor — no system actor appears as a primary initiator.
[PLACEHOLDER — Insert diagram here]

*Diagram 2 — Activity Diagram*
Title: Activity Diagram — View Lesson and Browse Attached Scenarios
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the student opening a lesson, reading the Lesson, and seeing the embedded scenario list when the lesson is scenario-bearing. Include an alternative flow for a lesson with no published scenarios.
[PLACEHOLDER — Insert diagram here]

*Diagram 3 — Wireframe*
Title: Wireframe — Lesson Page with Embedded Scenario List
Tool: Draw.io, Figma, or equivalent
Contents: Show the Lesson page as a scrollable instructional page with attached scenario cards displayed below the content when the lesson is scenario-bearing.
[PLACEHOLDER — Insert diagram here]

---

### 2.2 Browse Lesson Scenarios and Select Scenario Difficulty Level

| Field | Description |
|-------|-------------|
| **Use Case ID** | UC-DASH-04 |
| **Use Case Name** | Browse Lesson Scenarios and Select Scenario Difficulty Level |
| **Actor(s)** | Authenticated Student |
| **Preconditions** | Student is authenticated and is viewing a scenario-bearing Lesson page. Easy is available by default for every scenario. Medium and Hard may be locked depending on the student's completion status for the same scenario. |
| **Main Flow** | 1. Student views the Lesson's embedded scenario list.<br>2. System displays scenario cards under the selected lesson. Each scenario card shows scenario title, short narrative/skill focus, grading rules, completion status, retry indicators, and Easy/Medium/Hard difficulty states.<br>3. Student selects an available difficulty level from a scenario card.<br>4. System triggers UC-SCENE-01 to load the corresponding scenario instance.<br>5. Difficulty progression follows the scenario-specific mastery gate: Easy is initially available, Medium unlocks only after Easy completion for the same scenario, and Hard unlocks only after Medium completion for the same scenario. |
...
| **Alternative Flows** | AF1 — Locked difficulty selected: System does not load the scenario instance and displays the required prerequisite.<br>AF2 — Completed difficulty selected: System allows replay as a new session while preserving the existing completion record and prior logs.<br>AF3 — Lesson has no attached scenarios: System displays the lesson as reference content and does not render scenario difficulty controls. |
| **Functional Requirements** | FR-DASH-15: The Dashboard/Units tab shall allow a student to expand a learning unit card to display that unit's lessons.<br>FR-DASH-16: The scenario list embedded under a Lesson shall use the same scenario records, difficulty access rules, and completion data as the scenario session APIs.<br>FR-DASH-17: Display all three difficulty levels (Easy, Medium, Hard) for every scenario with their current access/status state: Available, Locked, In Progress, or Complete.<br>FR-DASH-18: Gate Medium behind completion of Easy for the same scenario and gate Hard behind completion of Medium for the same scenario.<br>FR-DASH-19: Display the completion status (Not Started / In Progress / Complete) for each difficulty level per scenario.<br>FR-DASH-20: Scenario selection shall not be globally gated by unit completion, quiz scores, manual approval, or completion of unrelated scenarios.<br>FR-DASH-21: After a primary scenario session starts, completes, fails, or is retried, the frontend shall update only the affected scenario card and difficulty-state entry in any already-loaded scenario list, including completion status, Review availability, First-Attempt Star status, active-session state, and next-difficulty unlock state, without forcing every scenario card in the list to reload. |

**UC-DASH-04 — Required Diagrams**

*Diagram 1 — Use Case Diagram*
Title: Use Case Diagram — Browse Lesson Scenarios and Select Scenario Difficulty Level
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the **Authenticated Student** actor as the initiating subject. Include use cases "Browse Lesson Scenarios" and "Select Difficulty Level" inside the system boundary. No system actor appears as a primary initiator.
[PLACEHOLDER — Insert diagram here]

*Diagram 2 — Activity Diagram*
Title: Activity Diagram — Browse Lesson Scenarios and Select Scenario Difficulty Level
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the Lesson scenario list, difficulty access check, selected available difficulty, and scenario-instance load decision.
[PLACEHOLDER — Insert diagram here]

*Diagram 3 — Wireframe*
Title: Wireframe — Lesson Scenario List with Difficulty Selection
Tool: Draw.io, Figma, or equivalent
Contents: Show a Lesson page with attached scenario cards and difficulty selectors on each scenario card.
[PLACEHOLDER — Insert diagram here]
...
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show the **Authenticated Student** actor as the initiating subject. Show "Load a Scenario Instance" inside the system boundary. Show the two preceding use cases (from Lesson path and Dashboard/Units tab path) as separate entry-point use cases that both include or precede "Load a Scenario Instance." Do not model logging as a student use case.
[PLACEHOLDER — Insert diagram here]
...
| **Main Flow** | 1. Student navigates to the Student Dashboard.<br>2. System retrieves the student's unit completion records, scenario completion records, streak data, first-attempt star flags, retry-count trends, Hard-level completion indicators, RTA progress, and session log aggregates from the database.<br>3. System renders dashboard summary indicators and one card for each published learning unit.<br>4. Each unit card displays unit title, short description, progress bar, lesson action, and an expand/collapse control.<br>5. Student expands a unit card.<br>6. System displays the lessons belonging to that unit directly below the expanded card.<br>7. Each lesson row displays lesson title, subtitle, completion/read status when applicable, and attached scenario count when applicable.<br>8. Student selects **View lesson** to open the lesson page and, for scenario-bearing lessons, the embedded scenario list.<br>9. System displays the active streak counter, first-attempt star indicators, retry-count trends, KPI summary indicators, and improvement indicators derived exclusively from logs. |
| **Postconditions** | Student sees current learning status, progress bars, KPI summary indicators, improvement indicators, expanded unit lesson lists, difficulty access states on scenario-bearing lessons, and review access for completed scenario difficulties. |
...
Tool: Draw.io (or equivalent UML-compliant tool)
Contents: Show dashboard load, unit cards rendering, unit expansion, lesson list display, lesson opening, scenario-bearing lesson display, difficulty selection, locked-state handling, and transition to scenario instance loading.
[PLACEHOLDER — Insert diagram here]

*Diagram 3 — Wireframe*
Title: Wireframe — Dashboard/Units Tab with Expanded Unit Card
Tool: Draw.io, Figma, or equivalent
Contents: Show unit cards, one expanded unit card with lesson rows underneath, a scenario-bearing Lesson page with scenario cards, difficulty selectors, completion indicators, retry indicators, and Review buttons.
[PLACEHOLDER — Insert diagram here]
...
| **Actor(s)** | Authenticated Student |
| **Preconditions** | The student is viewing the dashboard, a scenario-bearing Lesson page, or a scenario completion screen. At least one completed session exists for the selected scenario difficulty instance. |
...
| General Objective 5: Improve progress self-monitoring through visible, log-derived dashboard indicators | **Module 5: Progress Tracking and Self-Monitoring** | One-to-one objective-module | Session/step logging (FRG-LOG-01), log-derived dashboard indicators, unit-card progress display, expanded unit lesson lists, Review button visibility, Review Mode routing | 100% log-display match accuracy; SAR ≤ 20%; dashboard acceptance test set |
